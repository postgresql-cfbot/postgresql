...
static void save_to_tuplestore(PgFdwScanState *fsstate);
	...
	if (reinitialize_scan)
	{
		if (fsstate->streaming_fetch)
		{
			if (is_active_scan(fsstate))
			{
				...
				pgfdw_cancel_scan(fsstate->conn);
			}
...
init_scan(ForeignScanState *node)
{
	 ...
	 * different query than the current one. In the case of rescan, init_scan
	 * is called simultaneously for the same query so to avoid calling
	 * save_to_tuplestore in such cases, check if the queries are different
	 * and tuplestore is not already filled for this query.
	 */
	if (fsstate->conn_state->active_scan &&
		fsstate != fsstate->conn_state->active_scan)
		save_to_tuplestore(fsstate);
 ...
 */
static void
save_to_tuplestore(PgFdwScanState *fsstate)
{
	...
	/*
	 * Mark active_scan as NULL before draining.  If an error is thrown inside
	 * the loop (interrupts, fatal result), abort-time cleanup will not find a
	 * partially-drained scan marked as live.
	 */
	fsstate->conn_state->active_scan = NULL;
