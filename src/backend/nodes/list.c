 ...
 * caller. 'list' is assumed to be non-NIL, and 'pos' must not be past
 * the end-of-the-list-plus-one.
...
lappend_cell(List *list, ListCell *prev, void *datum)
{
	...
	new_cell = add_new_cell_after(list, prev);
 ...
 * destructively changed, list2 is not. Callers should be sure to use
 * the return value as the new pointer to the concatenated list: the 'list1'
