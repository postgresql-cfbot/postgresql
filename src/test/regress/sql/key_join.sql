...
-- ALTER POLICY must keep each stored proof's dependencies recorded, whether
-- the expression is re-supplied in the command or reloaded from the catalog.
-- Separate FK pairs let each probe target one expression's proof.
CREATE TABLE policy_alter_kj_uparent (id int PRIMARY KEY);
CREATE TABLE policy_alter_kj_uchild
(
    id int PRIMARY KEY,
    parent_id int NOT NULL REFERENCES policy_alter_kj_uparent (id)
);
CREATE TABLE policy_alter_kj_wparent (id int PRIMARY KEY);
CREATE TABLE policy_alter_kj_wchild
(
    id int PRIMARY KEY,
    parent_id int NOT NULL REFERENCES policy_alter_kj_wparent (id)
);
CREATE TABLE policy_alter_kj_guarded (id int);
ALTER TABLE policy_alter_kj_guarded ENABLE ROW LEVEL SECURITY;
CREATE POLICY policy_alter_kj ON policy_alter_kj_guarded
USING (EXISTS (
    SELECT 1
    FROM policy_alter_kj_uparent p
    -- accepted
    JOIN policy_alter_kj_uchild c FOR KEY (parent_id) -> p (id)
))
WITH CHECK (EXISTS (
    SELECT 1
    FROM policy_alter_kj_wparent p
    -- accepted
    JOIN policy_alter_kj_wchild c FOR KEY (parent_id) -> p (id)
));

-- Re-supplying USING re-proves it from source; WITH CHECK is reloaded from
-- the catalog and must keep its recorded proof dependencies.
ALTER POLICY policy_alter_kj ON policy_alter_kj_guarded
USING (EXISTS (
    SELECT 1
    FROM policy_alter_kj_uparent p
    -- accepted
...
-- rejected, reason: the re-supplied USING proof depends on the FK
ALTER TABLE policy_alter_kj_uchild
    ALTER CONSTRAINT policy_alter_kj_uchild_parent_id_fkey NOT ENFORCED;

-- rejected, reason: the reloaded WITH CHECK proof depends on the FK
ALTER TABLE policy_alter_kj_wchild
    ALTER CONSTRAINT policy_alter_kj_wchild_parent_id_fkey NOT ENFORCED;

-- Re-supplying WITH CHECK re-proves it from source; USING is reloaded.
ALTER POLICY policy_alter_kj ON policy_alter_kj_guarded
WITH CHECK (EXISTS (
    SELECT 1
    FROM policy_alter_kj_wparent p
...
-- rejected, reason: the reloaded USING proof depends on the FK
ALTER TABLE policy_alter_kj_uchild
    ALTER CONSTRAINT policy_alter_kj_uchild_parent_id_fkey NOT ENFORCED;

-- rejected, reason: the re-supplied WITH CHECK proof depends on the FK
ALTER TABLE policy_alter_kj_wchild
    ALTER CONSTRAINT policy_alter_kj_wchild_parent_id_fkey NOT ENFORCED;

-- Changing only the roles reloads both expressions from the catalog.
ALTER POLICY policy_alter_kj ON policy_alter_kj_guarded TO PUBLIC;
...
-- After dropping the policy, the same DDL succeeds.
DROP POLICY policy_alter_kj ON policy_alter_kj_guarded;
ALTER TABLE policy_alter_kj_uchild
    ALTER CONSTRAINT policy_alter_kj_uchild_parent_id_fkey NOT ENFORCED;
ALTER TABLE policy_alter_kj_wchild
    ALTER CONSTRAINT policy_alter_kj_wchild_parent_id_fkey NOT ENFORCED;
DROP TABLE policy_alter_kj_guarded,
           policy_alter_kj_uchild,
           policy_alter_kj_uparent,
           policy_alter_kj_wchild,
           policy_alter_kj_wparent;
...
-- Also leave a stored policy proof behind for the pg_upgrade dump tests:
-- its 'k' edges must order CREATE POLICY after the constraints it needs.
CREATE TABLE keyjoin_dump_guarded (id int, kdc_ref int);
ALTER TABLE keyjoin_dump_guarded ENABLE ROW LEVEL SECURITY;
CREATE POLICY keyjoin_dump_policy ON keyjoin_dump_guarded
USING (EXISTS (
    SELECT 1
    FROM keyjoin_dump_parent p
    -- accepted
    JOIN keyjoin_dump_child c FOR KEY (kdc_parent_id) -> p (kdp_id)
    WHERE c.kdc_id = keyjoin_dump_guarded.kdc_ref
));
