...
static void tidstore_iter_extract_tids(TidStoreIter *iter, BlocktableEntry val);
						...
						int num_offsets)
{
	...
	if (TidStoreIsShared(ts))
		shared_rt_set(ts->tree.shared, blkno, &page);
	else
		local_rt_set(ts->tree.local, blkno, &page);
...
TidStoreIsMember(TidStore *ts, ItemPointer tid)
{
	...
	if (TidStoreIsShared(ts))
		found = shared_rt_search(ts->tree.shared, blk, &page);
	else
		found = local_rt_search(ts->tree.local, blk, &page);
...
static inline bool
tidstore_iter_kv(TidStoreIter *iter, uint64 *key, BlocktableEntry *val)
{
...
TidStoreIterateNext(TidStoreIter *iter)
{
	...
	if (tidstore_iter_kv(iter, &key, &page))
	{
		{
