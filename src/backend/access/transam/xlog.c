							...
							bool fetching_ckpt, XLogRecPtr tliRecPtr)
{
	...
	for (;;)
	{
		...
		/*
		 * Check for the recovery pause here so that while waiting for the WAL
		 * if user requested for the recovery pause then we can update the
		 * recovery pause state.  So that the user can get confirmation of the
		 * recovery pause.
		 */
		if (((volatile XLogCtlData *) XLogCtl)->recoveryPauseState !=
			RECOVERY_NOT_PAUSED)
