						   ...
						   int *number_groupby_pathkeys)
{
	 ...
	 * as there's no chance of a Hash Aggregate plan as create_grouping_paths
							  ...
							  bool force_rel_creation)
{
	...
	if (can_sort && cheapest_total_path != NULL)
	{
		...
		if (numAggPathkeys > 0)
		{
			group_pathkeys = list_copy_head(root->group_pathkeys, root->num_groupby_pathkeys);
			orderAggPathkeys = list_copy_tail(root->group_pathkeys, root->num_groupby_pathkeys);
		}
		else
		{
			group_pathkeys = root->group_pathkeys;
