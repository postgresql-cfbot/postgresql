 ...
 *	dropped relations as well as relations dropped by other backends.  This is
 *	called as part of pre-commit processing.
 */
static void
gtr_process_invalidated_gtrs(void)
{
	 ...
	 * gtrs_dropped list.  As we scan gtrs_invalidated, more invalidation
	 * messages might arrive, and be added to the end of the list, so we need
	 * to be prepared for the list growing as we traverse it.
	 */
...
InitGlobalTempRelation(Relation relation)
{
					 ...
					 &relation->rd_locator, HASH_FIND, NULL) == NULL))
	{
		...
		if (RELKIND_HAS_TABLE_AM(relation->rd_rel->relkind))
		{
			...
			/* Update tempfrozenxid and tempminmxid for this backend */
			UpdateTempFrozenXidsForRel(relation->rd_rel->relfrozenxid,
									   (MultiXactId) relation->rd_rel->relminmxid);
		}
		else
		...
		/* Register ON COMMIT action, if it's DELETE ROWS (mustn't be DROP) */
...
GlobalTempRelationDropped(Oid relid)
{
	...
	/*
	 * Schedule any ON COMMIT action for the relation to be removed when the
	 * transaction commits.
	 */
