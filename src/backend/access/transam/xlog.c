 ...
 * data must be MAXALIGNed.  XLogInsertRecord fills in the xl_curr and
				 ...
				 uint8 flags)
{
	 ...
	 *
	 * Prior to PG11, we also used to store the link to the previous WAL record
	 * in the header. Apart from quickly locating the previous WAL record, it
	 * used to serve as an important safeguard against possible WAL corruption.
	 * During replay, we always used to check if the previous link stored in
	 * the  WAL record matches with the previous WAL record seen. In case of a
	 * mismatch, we could safely assume that the end-of-WAL has reached.
	 *
	 * There were no other significant benefit of the previous link though
	 * (except pg_rewind which can be handled in a different way). But by not
	 * tracking the previous link, we could now replace the current WAL
	 * position by an atomic integer, thus greatly reducing contention on the
	 * wal insertion spinlock (in fact, the spinlock no longer exists).
	 */
	if (isLogSwitch)
	...
	/* Store the WAL position in the record. */
	rechdr->xl_curr = StartPos;

	if (inserted)
	{
		/*
		 * Now that xl_curr has been filled in, calculate CRC of the record
		 * header.
		 */
