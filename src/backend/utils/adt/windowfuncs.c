...
/*
 * Returns typ of nth argument of Window function. For reduce of
 * overhead it uses result cache in window's proxy object.
 */
static Oid
...
/*
 * SQL wrapper of WinGetFuncArgInPartition function
 */
Datum
...
/*
 * SQL wrapper of WinGetFuncArgInFrame function
 */
Datum
...
/*
 * SQL wrapper of WinGetFuncArgCurrent function
 */
Datum
								...
								int *realsize)
{
	if(typlen != -1)
	{
		...
		if (typlen > 1024)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("size of value (%d bytes) is greather than limit (1024 bytes)",
							typlen)));
	...
	else
	{
		if (!isnull)
		{
			...
			*realsize = size;
			...
			/*
			 * Increase size about 1/3 for reserve, because the size
			 * of varlena objects can be mutable, and we estimate size
			 * just from first value
			 */
			size += size / 3;
		...
		{
			/* by default we allocate 32 bytes */
			*realsize = 0;
...
get_partition_context(FunctionCallInfo fcinfo, bool write_mode)
{
	...
	else
	{
		...
		if (minsize < VARLENA_MINSIZE)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("the value of minsize argument is too low"),
					 errhint("Minimum value is %d (bytes)", VARLENA_MINSIZE)));
	}
	...
	/* WinGetPartitionLocalMemory doesn't support realloc */
		...
		if (realsize > pcontext->allocsize)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 ...
					 errdetail("the size of value (%d bytes) is longer then size of buffer (%d bytes)",
							   realsize, pcontext->allocsize),
