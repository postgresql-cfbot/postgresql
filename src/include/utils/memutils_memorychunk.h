 ...
 * to store a reference back to the block the which the given chunk is
 * allocated on and also an additional 30-bits to store another value such as
...
/* A magic number for storing in the free bits of an external chunk */
...
#define HdrMaskIsExternal(hdrmask) \
	((hdrmask) & (1 << MEMORYCHUNK_EXTERNAL_BASEBIT))
