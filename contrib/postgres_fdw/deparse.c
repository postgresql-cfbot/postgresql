				 ...
				 bool qualify_col)
{
	...
	if (rte->rtekind == RTE_FUNCTION)
	{
		if (varattno == 0)
		{
			 ...
			 * remote outer join we emit ourselves.  However we still have to
			 * emit the wrapper unconditionally: it is a no-op for INNER-join
			 * rows, but it keeps the deparsed SQL correct if a higher-level
			 * outer join propagates a NULL row through the foreign scan's tuple.
			 */
			if (qualify_col)
			{
