...
/*
 * We need variable sized chunks, because we might be asked to store gigantic
 * tuples.  To avoid the locking contention that would arise if we had to read
 * the chunk header on disk while concurrent readers queue up to find out
 * where the next chunk starts, we'll track the position of chunk size changes
 * in shared memory.
 */
typedef struct ChunkExpansionLogEntry
{
	BlockNumber	position;
	BlockNumber	chunk_size;
} ChunkExpansionLogEntry;

/* The maximum number of chunk size expansions that there could possibly be. */
#define CHUNK_EXPANSION_LOG_SIZE (sizeof(BlockNumber) * CHAR_BIT)
...
typedef struct SharedTuplestoreParticipant
{
	...
	/* Track the chunk size if it increases in this participant's file. */
	ChunkExpansionLogEntry chunk_expansion_log[CHUNK_EXPANSION_LOG_SIZE];
	int			chunk_expansions;	/* Num expansions while writing. */
	int			chunk_expansion;	/* Num expansions while reading. */
	int			chunk_pages;		/* Chunk size while reading. */
...
struct SharedTuplestore
{
	...
	char		name[32];		/* A name for this tuplestore. */
...
struct SharedTuplestoreAccessor
{
	...
	int			read_ntuples_available;	/* The number of tuples we can read. */
	int			read_ntuples;	/* How many tuples have we read so far? */
	...
	int			write_pages;	/* The current chunk size in pages. */
 ...
 * applications.
 *
 * The caller should supply a SharedFileSet, which is essentially a directory
			   ...
			   const char *name)
{
	...
	{
		LWLockInitialize(&sts->participants[i].mutex,
						 LWTRANCHE_SHARED_TUPLESTORE);
		sts->participants[i].read_page = 0;
		sts->participants[i].chunk_pages = STS_MIN_CHUNK_PAGES;
		sts->participants[i].chunk_expansions = 0;
...
sts_flush_chunk(SharedTuplestoreAccessor *accessor)
{
	...
	size = accessor->write_pages * BLCKSZ;
...
sts_reinitialize(SharedTuplestoreAccessor *accessor)
{
	 ...
	 * initial chunk size to the minimum (any increases from that size will
	 * be recorded in chunk_expansion_log).
	 */
	for (i = 0; i < accessor->sts->nparticipants; ++i)
	{
		accessor->sts->participants[i].read_page = 0;
		accessor->sts->participants[i].chunk_pages = STS_MIN_CHUNK_PAGES;
	}
}
			 ...
			 MinimalTuple tuple)
{
	...
	if (accessor->write_file == NULL)
	{
		...
		participant->chunk_expansions = 0;
		accessor->write_pages = STS_MIN_CHUNK_PAGES;
	}
	...
	if (accessor->write_pointer + size >= accessor->write_end)
	{
		...
		if (accessor->write_pointer + size >= accessor->write_end)
		{
			...
			p = &accessor->sts->participants[accessor->participant];

			/*
			 * If we need to increase the chunk size, we'll log the change so
			 * that readers can see it.
			 */
			if (accessor->write_pages < pages_needed)
			{
				ChunkExpansionLogEntry *entry;

				/*
				 * We'll grow the chunk size geometrically, so that a small
				 * fixed-size expansion log is sufficient.
				 */
				while (accessor->write_pages < pages_needed)
					accessor->write_pages *= 2;

				/* Record the size change at this position. */
				entry = &p->chunk_expansion_log[p->chunk_expansions];
				entry->position = accessor->write_page;
				entry->chunk_size = accessor->write_pages;
				++p->chunk_expansions;
			}
			...
			accessor->write_chunk = (SharedTuplestoreChunk *)
				MemoryContextAllocZero(accessor->context,
									   accessor->write_pages * BLCKSZ);
			accessor->write_chunk->ntuples = 0;
			accessor->write_chunk->npages = accessor->write_pages;
			accessor->write_pointer = &accessor->write_chunk->data[0];
			accessor->write_end = (char *)
				accessor->write_chunk + BLCKSZ * accessor->write_pages;
		}
	}
...
sts_read_tuple(SharedTuplestoreAccessor *accessor, void *meta_data)
{
	...
	if (BufFileRead(accessor->read_file,
		...
		ereport(ERROR,
				(errcode_for_file_access(),
				 errmsg("could not read from shared tuplestore temporary file"),
				 errdetail("Short read while reading size")));
	if (size > accessor->read_buffer_size)
	{
		...
		accessor->read_buffer =
			MemoryContextAlloc(accessor->context,
							   Max(size, accessor->read_buffer_size * 2));
	}
	if (BufFileRead(accessor->read_file,
					accessor->read_buffer + sizeof(uint32),
					size - sizeof(uint32)) != size - sizeof(uint32))
		ereport(ERROR,
				(errcode_for_file_access(),
				 errmsg("could not read from shared tuplestore temporary file"),
				 errdetail("Short read while reading tuple")));
...
sts_parallel_scan_next(SharedTuplestoreAccessor *accessor, void *meta_data)
{
	...
	for (;;)
	{
		...
		LWLockAcquire(&p->mutex, LW_EXCLUSIVE);
		eof = p->read_page >= p->npages;
		if (!eof)
		{
			/*
			 * Figure out how big this chunk is.  It will almost always be the
			 * same as the last chunk loaded, but if there is an entry in the
			 * chunk expansion log for this page then we need to read the new
			 * size.  This avoids the need to do IO to adjust the read head,
			 * so we don't need to hold up concurrent readers.
			 */
			read_page = p->read_page;
			if (p->chunk_expansion < p->chunk_expansions &&
				p->chunk_expansion_log[p->chunk_expansion].position == p->read_page)
			{
				p->chunk_pages =
					p->chunk_expansion_log[p->chunk_expansion++].chunk_size;
			}
		...
		if (!eof)
		{
			...
			/* Sanity check: see if the chunk size matches our expectation. */
			if (chunk_header.npages != chunk_pages)
				elog(ERROR, "expected %d pages but chunk header says %d",
					 chunk_pages, chunk_header.npages);
