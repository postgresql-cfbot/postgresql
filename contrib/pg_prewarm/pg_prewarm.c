...
pg_prewarm(PG_FUNCTION_ARGS)
{
	...
	{
		/*
		 * Normally, we treat a missing fork as an error, but in a parallel
		 * worker, it can happen for a global temporary relation that hasn't
		 * been used, and so has not been initialized, which isn't an error.
		 */
