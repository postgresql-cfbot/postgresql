...
static StdRdOptions *parseRelCompressOptions(HeapTuple tuple);
...
static StdRdOptions *
parseRelCompressOptions(HeapTuple tuple)
{
	...
	if (pgclassdesc == NULL)
	{
		pgclassdesc = CreateTemplateTupleDesc(Natts_pg_class);
		pgclassdesc->tdtypeid = RECORDOID;	/* not right, but we don't care */
		pgclassdesc->tdtypmod = -1;

		for (int i = 0; i < Natts_pg_class; i++)
		{
			memcpy(TupleDescAttr(pgclassdesc, i), &Desc_pg_class[i], ATTRIBUTE_FIXED_PART_SIZE);
			/* make sure attcacheoff is valid */
			TupleDescAttr(pgclassdesc, i)->attcacheoff = -1;
		}

		/* initialize first attribute's attcacheoff, cf RelationBuildTupleDesc */
		TupleDescAttr(pgclassdesc, 0)->attcacheoff = 0;
	}
	...
	switch (classForm->relkind)
	{
		...
		case RELKIND_MATVIEW:
			options = (StdRdOptions *) default_reloptions(datum, false, RELOPT_KIND_HEAP);
			break;
		default:
						  ...
						  Oid src_tsid, Oid dst_tsid)
{
	...
	foreach(cell, rlocatorlist)
	{
		...
		dstrlocator.compressOpt = srcrlocator.compressOpt;
							   ...
							   char *srcpath)
{
	...
	relinfo = (CreateDBRelInfo *) palloc0(sizeof(CreateDBRelInfo));
	...
	options = parseRelCompressOptions(tuple);
	if(options)
	{
		relinfo->rlocator.compressOpt.algorithm = options->compress.compresstype;
		relinfo->rlocator.compressOpt.level = options->compress.compresslevel;
		relinfo->rlocator.compressOpt.chunks_pre_block = BLCKSZ / options->compress.compress_chunk_size;
		relinfo->rlocator.compressOpt.prealloc_chunks = (uint8) options->compress.compress_prealloc_chunks;
	}
