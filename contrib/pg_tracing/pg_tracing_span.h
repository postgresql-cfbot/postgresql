/*-------------------------------------------------------------------------
 * contrib/pg_tracing/pg_tracing_span.h
 * 		Header for span.
 *
 * IDENTIFICATION
 *	  contrib/pg_tracing/pg_tracing_span.h
 ...
 */
typedef struct pgTracingStartTime
{
...
typedef struct pgTracingTraceContext
{
	...
	int			root_span_index;	/* span index for the root span */
...
extern void begin_span(const pgTracingTraceContext * trace_context, Span * span, SpanType type, uint64 parent_id, uint64 query_id, const int64 *start_span);
extern void end_span(Span * span, const int64 *end_time);
extern void set_span_start(const pgTracingTraceContext * trace_context, Span * span, int64 start_ns_time);
