...
free_session_variable_value(SVariable svar)
{
	...
	/*
	 * pfree() can technically error out, so treat it at the very end, making
	 * sure that we won't leave the svar pointing to a corrupted pointer in
	 * case of error.
	 */
	if (!svar->freeval)
		svar->value = (Datum) 0;
...
sync_sessionvars_all(bool filter_lxid)
{
	...
	foreach(l, xact_recheck_varids)
	{
		 ...
		 */
		if (filter_lxid)
			xact_recheck_varids = foreach_delete_current(xact_recheck_varids,
														 l);
	}

	/*
	 * If caller didn't ask to filter the list, all items have been processed
	 * so we can simply destroy the list.
	 */
	if (!filter_lxid)
	{
		list_free(xact_recheck_varids);
...
set_session_variable(SVariable svar, Datum value, bool isnull, bool init_mode)
{
	...
	/*
	 * free_session_variable_value can technically error out, but we may have
	 * already allocated the new value in SVariableMemoryContext, so make sure
	 * that we assign the value (and the rest of the fields) to the svar before
	 * throwing the error.
	 */
	PG_TRY();
	{
...
#define NUM_PG_DEBUG_SHOW_USED_SESSION_VARIABLES_ATTS 10
	...
	elog(DEBUG1, "pg_debug_show_used_session_variables start");
		...
		while ((svar = (SVariable) hash_seq_search(&status)) != NULL)
		{
			...
			if (HeapTupleIsValid(tp))
			{
				...
				values[8] = BoolGetDatum(
										 pg_variable_aclcheck(svar->varid, GetUserId(), ACL_SELECT) == ACLCHECK_OK);

				values[9] = BoolGetDatum(
										 pg_variable_aclcheck(svar->varid, GetUserId(), ACL_UPDATE) == ACLCHECK_OK);
	...
	elog(DEBUG1, "pg_debug_show_used_session_variables end");
