 ...
 * based on the protocol message type. Currently, it is a basic prototype do demonstrate
...
zpq_build_msg_compression_map(ZpqStream * zpq)
{
	...
	{
		if (pg_strcasecmp(supported_algorithms[zpq->compressors[i].impl], "zstd") == 0)
		{
			/* compress CopyData and DataRow messages with ZSTD */
			zpq->compressor_by_msg_type['d'] = i;
			zpq->compressor_by_msg_type['D'] = i;
		}
		else if (pg_strcasecmp(supported_algorithms[zpq->compressors[i].impl], "zlib") == 0)
		{
			/* compress Query messages with ZLIB (just an example) */
...
zpq_read(ZpqStream * zpq, void *dst, size_t dst_size)
{
	...
	while (dst_pos == 0)
	{
		...
		while (zpq->rx_msg_bytes_left > 0 && (zpq_buf_unread(&zpq->rx_in) >= zpq->rx_msg_bytes_left + 5))
		{
			...
			if (zpq->is_decompressing || zpq_is_compressed_msg(msg_type) || zpq_is_set_compression_msg(msg_type))
			{
				/*
				 * cannot proceed further, encountered compression toggle
				 * point
...
zpq_parse_compression_setting(char *val, zpq_compressor * *compressors, size_t *n_compressors)
{
	...
	*n_compressors = 0;
	...
	return zpq_deserialize_compressors(val, compressors, n_compressors);
}
