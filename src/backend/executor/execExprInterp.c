...
ExecInterpExpr(ExprState *state, ExprContext *econtext, bool *isnull)
{
	...
	strictfail:
		...
		EEO_CASE(EEOP_PARAM_VARIABLE)
		{
			 ...
			 * Because returned value can be used (without an assignement)
			 * after the referenced session variables is updated, we have
			 * to return copy of stored value every time. This is not an
			 * issue for local (plpgsql) variables.
			 */
			*op->resvalue = CopySessionVariableWithTypeCheck(op->d.vparam.varid,
															 op->resnull,
															 op->d.vparam.vartype);
			EEO_NEXT();
		}
