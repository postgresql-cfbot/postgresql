...
StartupXLOG(void)
{
	...
	if (ArchiveRecoveryRequested)
	{
		 ...
		 *
		 * As a compromise, we rename the last segment with the new timeline and
		 * .partial suffix, and archive it. The timeline is added in case there
		 * are multiple promotions from the same timeline before a stable
			...
			XLogArchivingActive())
		{
			...
			if (!XLogArchiveIsReadyOrDone(origfname))
			{
				...
				snprintf(partialfname, MAXFNAMELEN, "%s.%08X.partial",
						 origfname, ThisTimeLineID);
				snprintf(partialpath, MAXPGPATH, "%s.%08X.partial", origpath,
						 ThisTimeLineID);
