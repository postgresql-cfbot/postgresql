 ...
 * being joined. partexprs and nullable_partexprs are arrays containing part_scheme->partnatts
 * elements each. Each of these elements is a list of partition key expressions.
 * For a base relation each list in partexprs contains only one expression and
 * nullable_partexprs is not populated. For a join relation, partexprs and
 * nullable_partexprs contain partition key expressions from non-nullable and
 * nullable relations resp. Lists at any given position in those arrays
 * together contains as many elements as the number of joining relations.
 *----------
 */
