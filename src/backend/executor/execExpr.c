					...
					case PARAM_VARIABLE:
						{
							...
							else
							{
								 ...
								 * without query executor start (plpgsql
								 * simple expr evaluation), then the array
								 * es_session_variables is null. In this case
								 * we need to use direct access to session
								 * variables. The values are not protected by
								 * using copy, but it is not problem (we don't
								 * need to emulate stability of the value).
								 *
