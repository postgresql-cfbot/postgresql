...
static List *remove_functionally_dependant_clauses(PlannerInfo *root,
									  List *clauselist);
...
remove_useless_groupby_columns(PlannerInfo *root)
{
	...
	parse->groupClause = remove_functionally_dependant_clauses(root,
														parse->groupClause);
}
...
remove_useless_distinct_columns(PlannerInfo *root)
{
	...
	/* No chance to do anything if there are less than two DISTINCT items */
	if (list_length(parse->distinctClause) < 2)
		return;

	parse->distinctClause = remove_functionally_dependant_clauses(root,
													parse->distinctClause);
}

/*
 * remove_functionally_dependant_clauses
 *		Processes clauselist and removes any items which are deemed to be
 *		functionally dependant on other clauselist items.
 *
 ...
 */
static List *
remove_functionally_dependant_clauses(PlannerInfo *root,
									  List *clauselist)
{
	Query	   *parse = root->parse;
