...
preprocess_targetlist(PlannerInfo *root)
{
	...
	if (command_type == CMD_MERGE)
	{
		 ...
		 * identify the rows to be updated or deleted, with different handling
		 * for partitioned tables.
		 */
		rewriteTargetListMerge(parse, target_relation);

		/*
		 * For MERGE command, handle targetlist of each MergeAction
		 * separately. Give the same treatment to MergeAction->targetList as
		 * we would have given to a regular INSERT/UPDATE/DELETE.
		 */
