...
MainLoop(FILE *source)
{
	...
	while (successResult == EXIT_SUCCESS)
	{
		...
		if (pset.cur_cmd_interactive)
		{
			...
			/*
			 * Trim leading whitespace, only for purposes of recognizing the
			 * command
			 */
			while (isspace((unsigned char) *first_word))
				++first_word;

			/* Search for the words we recognize. */
			...
			{
				if (query_buf->len != 0)
					puts(_("Use \\q to quit or press control-C to clear the input buffer."));
				else
				{
					free(line);
					fflush(stdout);
