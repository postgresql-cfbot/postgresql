...
-- Test changing column data type to constrained domain
...
ALTER TABLE t22 ALTER COLUMN a SET DATA TYPE domain2; -- table rewrite
ALTER TABLE t22 ALTER COLUMN a SET DATA TYPE domain3; -- table rewrite

ALTER TABLE t22 ALTER COLUMN c SET DATA TYPE domain4; -- no table rewrite
ALTER TABLE t22 ALTER COLUMN c SET DATA TYPE domain6; -- no table rewrite
ALTER TABLE t22 ALTER COLUMN col1 SET DATA TYPE domain5 USING col1::int4[]::domain5; -- no table rewrite
UPDATE t22 set col1 = '{-2,null}';
ALTER TABLE t22 ALTER COLUMN col1 SET DATA TYPE domain5 USING col1::int4[]::domain5; -- no table rewrite
ALTER TABLE t22 ALTER COLUMN a SET DATA TYPE domain1; -- no table rewrite
ALTER TABLE t22 ALTER COLUMN b SET DATA TYPE domain2 USING 15; -- table rewrite, ok
ALTER TABLE t22 DROP COLUMN a, DROP COLUMN c, ALTER COLUMN b SET DATA TYPE domain1, ADD COLUMN col2 INT DEFAULT 11; -- no table rewrite, ok

CREATE FUNCTION domain_test() RETURNS bool language plpgsql AS $$ BEGIN DELETE FROM t22; return true; END$$;
CREATE DOMAIN domain7 AS INT CHECK (domain_test());
ALTER TABLE t22 ALTER COLUMN b SET DATA TYPE domain7; -- table rewrite
TABLE t22;
DROP TABLE t22;
DROP DOMAIN domain1, domain2, domain3, domain4, domain5, domain6, domain7;
DROP FUNCTION domain_test();
