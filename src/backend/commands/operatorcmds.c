...
static Oid	ValidateOperatorDefined(List *name,
									Oid leftTypeId,
									Oid rightTypeId);
...
/*
 * Looks up and returns an oid for an operator given a possibly-qualified name
 * and left and right type IDs, verifies the operator is defined (non-shell),
 * and owned by the current user. Raises an error otherwise.
 */
static Oid
ValidateOperatorDefined(List *name,
						Oid leftTypeId,
						Oid rightTypeId)
{
	Oid			oid;
	...
	if (!OidIsValid(oid))
		ereport(ERROR,
				(errcode(ERRCODE_UNDEFINED_FUNCTION),
				 errmsg("operator %s does not exist",
						NameListToString(name))));

	if (!defined)
		ereport(ERROR,
				(errcode(ERRCODE_UNDEFINED_FUNCTION),
				 errmsg("operator %s is undefined shell operator",
						NameListToString(name))));
 ...
 * COMMUTATOR and NEGATOR, HASHES, and MERGES can be set if they have not been set
 * previously.
 */
ObjectAddress
AlterOperator(AlterOperatorStmt *stmt)
{
	...
	if (commutatorName)
	{
		...
		commutatorOid = ValidateOperatorDefined(commutatorName,
												oprForm->oprright,
												oprForm->oprleft);
	...
	if (negatorName)
	{
		negatorOid = ValidateOperatorDefined(negatorName,
											 oprForm->oprleft,
											 oprForm->oprright);
	...
	/*
	 * check that we're not changing any existing values that might be
	 * depended on elsewhere and may be expected to never change, while
	 * allowing no-ops.
	 */
	...
	if (updateHashes && oprForm->oprcanhash && !canHash)
	{
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
				 errmsg("operator attribute \"hashes\" cannot be disabled after it has been enabled")));
	}
	...
	if (updateMerges && oprForm->oprcanmerge && !canMerge)
	{
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
				 errmsg("operator attribute \"merges\" cannot be disabled after it has been enabled")));
	}
