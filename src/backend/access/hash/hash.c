...
hashgetbatch(IndexScanDesc scan, IndexScanBatch priorbatch, ScanDirection dir)
{
	...
	if (priorbatch == NULL)
	{
		...
		/* Initialize the scan, and get the first batch of matching items */
		batch = _hash_first(scan, dir);
	}
	else
	{
		/* Get batch positioned after caller's batch (in direction 'dir') */
		batch = _hash_next(scan, dir, priorbatch);
	}

	/*
	 * When the batch's page has no next/previous page for _hash_next to step
	 * to in a direction, the scan already ended on the returned batch; mark
	...
	if (batch)
	{
		...
		if (!BlockNumberIsValid(hashbatch->nextPage))
			batch->knownEndForward = true;
		if (!BlockNumberIsValid(hashbatch->prevPage))
			batch->knownEndBackward = true;
	}
