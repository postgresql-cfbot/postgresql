							   ...
							   List **considered_relids)
{
	...
	foreach(lc, indexjoinclauses)
	{
		 ...
		 * so we can't use foreach here.  We do not need to visit such
		 * newly-added entries within this loop.  No real harm would be done
		 * if we did, since the subset check would reject them; but it would
		 * waste some cycles.
		 */
