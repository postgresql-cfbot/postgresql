...
XLogPrefetcherNextBlock(uintptr_t pgsr_private, XLogRecPtr *lsn)
{
	...
	for (;;)
	{
		 ...
		 * set to PANIC, which might otherwise cause XLogPageRead() to panic on
		 * some future page, and FinishWalRecovery() determining where to start
		 * writing WAL next, which depends on the contents of the reader's
		 * internal buffer after reading one record.
