 ...
 * hurt performance.  Note that this threshold value must always be lesser
 * than PGPROC_MAX_CACHED_SUBXIDS.
 */
 ...
 * to be acquired is reduced.  We try to do this iff the transaction id and
 * subtransaction ids whose status needs to be updated matches with the one
 * we have in PGPROC/PGXACT.  The reason of this restriction is that this
 * optimization relies on subxids stored in PGPROC.  We skip it if a process
 * has XIDs on more than one CLOG page, or on a different CLOG page than
 * processes already waiting for a group update.  This latter condition has a
 * race condition (see TransactionGroupUpdateXidStatus) but the worst thing
 * that happens if we mess up is a small loss of efficiency; the intent is to
 * avoid having the leader access pages it wouldn't otherwise need to touch.
 */
static void
						   ...
						   bool all_xact_same_page)
{
	if (all_xact_same_page &&
		...
		memcmp(subxids, MyProc->subxids.xids, nsubxids * sizeof(TransactionId)) == 0)
	{
		/*
