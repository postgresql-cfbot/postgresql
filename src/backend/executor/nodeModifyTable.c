 ...
 */
static TupleTableSlot *
ExecMerge(ModifyTableContext *context, ResultRelInfo *resultRelInfo, bool canSetTag)
{
	...
	{
		matched = false;
		/* we don't need this for NOT MATCHED actions */
		tupleid = NULL;
	}
...
ExecModifyTable(PlanState *pstate)
{
	...
	for (;;)
	{
		...
		if (AttributeNumberIsValid(node->mt_resultOidAttno))
		{
			...
			if (isNull)
			{
				...
				if (operation == CMD_MERGE)
				{
					...
					ExecMerge(&context, node->resultRelInfo, node->canSetTag);
					continue;
				}
		...
		switch (operation)
		{
			...
			case CMD_MERGE:
				slot = ExecMerge(&context, resultRelInfo, node->canSetTag);
				break;
