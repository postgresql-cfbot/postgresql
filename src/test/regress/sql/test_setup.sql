...
BEGIN
    ...
    -- pg_event_trigger_ddl_commands() can raise before yielding a row (e.g.
    -- an unsupported object type), so the whole loop -- not just each row --
    -- must be inside the handler; a gap on one row of a multi-row event
    -- therefore also discards verification already done for earlier rows in
    -- that event. Accepted trade-off for this test convenience. A genuine
    -- round-trip mismatch (SQLSTATE P0004) still propagates.
    BEGIN
        ...
        LOOP
            IF r.command_tag LIKE 'CREATE %' THEN
                ...
                IF EXISTS (
                    ...
                    -- pg_get_<type>_ddl() is SETOF text; aggregate all rows
                    -- in emission order instead of only capturing the first.
                    EXECUTE format(
                        'SELECT string_agg(ddl, E''\n'' ORDER BY ord)
                           FROM pg_get_%s_ddl(%L) WITH ORDINALITY AS t(ddl, ord)',
                        obj_type, r.object_identity)
                        INTO original;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            IF SQLSTATE = 'P0004' THEN
                RAISE;
            END IF;
            RAISE WARNING 'regress_ddl_roundtrip_trigger_func: skipping DDL round-trip check (%): %',
                SQLSTATE, SQLERRM;
    END;
END;
$$;
