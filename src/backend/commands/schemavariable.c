...
free_schema_variable(SchemaVariable svar, bool force)
{
		...
		svar->creating_subid != (current_subid = GetCurrentSubTransactionId()))
	{
		...
		/*
		 * In this moment, we would to register or update (change the
		 * creating_subid) OnCommit event. Now, some rollback is possible, and
		 * on commit action is necessary to release stacked variable.
		 */
...
rollback_transact_schema_variable(SchemaVariable svar)
{
	...
	/*
	 * allow reinit when variable is not valid and creating_subid is
	 * InvalidSubTransactionId - the variable was not initialized before
	 * reverted subtransaction.
	 */
