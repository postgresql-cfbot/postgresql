		...
		for (ctrlc = 0; ctrlc < 8 && sp < srcend && dp < destend; ctrlc++)
		{
			...
			if (ctrl & 1)
			{
				...
				while (off < len)
				{
					 ...
					 * Consider input: "112341234123412341234" At byte with
					 * position 5 we have match with length 16 and offset 4:
					 * 11234M(len=16, off=4). We are decoding first period of
					 * match and rewrite match 112341234M(len=12, off=8). Same
					 * match is now at position 9, it aims to same start byte
					 * of output, but from another position: offset is
					 * doubled. We iterate through this offset growth until we
					 * can proceed to usual memcpy(). If we would try to
					 * decode the match at byte 5 (len=16, off=4) by memmove()
					 * we would issue memmove(5, 1, 16) which would produce
