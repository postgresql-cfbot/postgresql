...
copy_table(Relation rel)
{
	 ...
	 *
	 * Note that "qual" can also be NIL due to the fact that we didn't check
	 * the publication quals. In such a case, "pubnames" is valid and we
	 * assume that the publication filters will be examined by the publisher.
	 * The "COPY <table> TO ..." form is not appropriate if the publisher
	 * might need to construct the quals.
	 */
	...
	else
	{
		...
		if (qual != NIL)
		{
			...
			/*
			 * fetch_remote_table_info() should only return qual for older
			 * publisher version. Publishers of more recent postgres versions
			 * do construct the filter themselves.
			 */
			Assert(walrcv_server_version(LogRepWorkerWalRcvConn) < 160000);
		...
		{
			Assert(qual == NIL);
			Assert(lrel.pubnames);

			/*
			 * Tell the publisher which publications we are interested in.
			 * Publishers of the later versions do need this information to
			 * construct the query filter and to check publication privileges.
			 */
