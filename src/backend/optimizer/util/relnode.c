					...
					RelOptInfo *input_rel)
{
	...
	foreach(vars, input_rel->reltarget->exprs)
	{
		 ...
		 * a Var or ConvertRowtypeExpr. For either of those, find the original
							 ...
							 JoinType jointype)
{
	 ...
	 * not partitioned or they do not have same partitioning scheme or if
	 * there is no equi-join between partition keys.
	 *
	 ...
	 * keys. Thus every pair of joining relations we see for this join should
	 ...
	 * expressions from both the relations. For tables A and B partitioned by
	 ...
	 * like (A LEFT JOIN B on (A.a = B.b) LEFT JOIN C ON B.b = C.c. If we do
	 * not include B.b as partition key expression for (AB), it prohibits us
	 * from using partition-wise join when joining (AB) with C as there is no
	 * equi-join between partition keys of joining relations. If the equality
	 * operator is strict, two NULL values are never equal and no two rows
	 * from mis-matching partitions can join. Hence if the equality operator
	 * used in B.b = C.c is strict, it's safe to use B.b as partition key
	 * expression of (AB), while joining with C even though rows in (AB) are
	 * not strictly partitioned by B.b. So, we separate non-nullable and
	 * nullable partition keys, and use nullable partition keys when the
	 * equality operator is strict.
	 *
	 * For a FULL OUTER join, both sides can be nullable, so a FULL OUTER join
	 * has only nullable partition keys. A LEFT OUTER JOIN has non-nullable
	 * partition keys if the outer side has those. It will always have
	 * nullable partition keys. An INNER join will have nullable partition
	 * keys if either of the joining relations has those. Same for
	 * non-nullable partition keys. RIGHT OUTER JOIN do not appear here since
	 * planner converts that into LEFT OUTER JOIN before calling this
	 * function.
	 */
	for (cnt = 0; cnt < partnatts; cnt++)
	{
		List	   *outer_partexprs = NIL;
		List	   *outer_nullable_partexprs = NIL;
		List	   *inner_partexprs = NIL;
		List	   *inner_nullable_partexprs = NIL;
		List	   *partexprs = NIL;
		List	   *nullable_partexprs = NIL;

		if (outer_rel->partexprs)
			outer_partexprs = list_copy(outer_rel->partexprs[cnt]);

		if (outer_rel->nullable_partexprs)
			outer_nullable_partexprs = list_copy(outer_rel->nullable_partexprs[cnt]);

		if (inner_rel->partexprs)
			inner_partexprs = list_copy(inner_rel->partexprs[cnt]);

		if (inner_rel->nullable_partexprs)
			inner_nullable_partexprs = list_copy(inner_rel->nullable_partexprs[cnt]);

		switch (jointype)
		{
			case JOIN_INNER:
				partexprs = list_concat(outer_partexprs, inner_partexprs);
				nullable_partexprs = list_concat(outer_nullable_partexprs,
												 inner_nullable_partexprs);
				break;

			case JOIN_SEMI:
			case JOIN_ANTI:
				partexprs = outer_partexprs;
				nullable_partexprs = outer_nullable_partexprs;
				break;

			case JOIN_LEFT:
				partexprs = outer_partexprs;
				nullable_partexprs = list_concat(inner_partexprs,
												 outer_nullable_partexprs);
				nullable_partexprs = list_concat(nullable_partexprs,
												 inner_nullable_partexprs);
				break;

			case JOIN_FULL:
				nullable_partexprs = list_concat(outer_partexprs,
												 inner_partexprs);
				nullable_partexprs = list_concat(nullable_partexprs,
												 outer_nullable_partexprs);
				nullable_partexprs = list_concat(nullable_partexprs,
		...
		if (partexprs)
		{
			...
			if (!joinrel->partexprs)
			{
				joinrel->partexprs = (List **) palloc0(sizeof(List *) * partnatts);
				Assert(cnt == 0);
			}
			joinrel->partexprs[cnt] = partexprs;
		}

		if (nullable_partexprs)
		{
			/* First time through allocate the array. */
			if (!joinrel->nullable_partexprs)
			{
				joinrel->nullable_partexprs = (List **) palloc0(sizeof(List *) *
																partnatts);
				Assert(cnt == 0);
			}
