...
BootStrapKmgr(void)
{
	...
	int			key_lens[KMGR_MAX_INTERNAL_KEYS];
	int			nkeys;
	...
	/*
	 * Copy cluster file encryption keys from an old cluster? This is
	 * useful for pg_upgrade upgrades where the copied database files
	 * are already encrypted using the old cluster's DEK keys.
	 */
	...
	/*
	 * Get key encryption key (KEK) from the cluster_key command.  The
	 * cluster key command might need to check for the existence of files
	 * in the live directory, e.g., PIV, so run this _after_ copying the
	 * directory in place.
	 */
	...
	if (bootstrap_old_key_datadir == NULL)
	{
		...
		for (int id = 0; id < KMGR_MAX_INTERNAL_KEYS; id++)
		{
			...
			/* generate a DEK */
			key = generate_crypto_key(encryption_methods[bootstrap_file_encryption_method].bit_length / 8);
	...
	nkeys = kmgr_read_wrapped_data_keys(LIVE_KMGR_DIR, keys_wrap, key_lens);
...
InitializeKmgr(void)
{
	 ...
	 * the new directory is partly written, or NEW wasn't renamed to LIVE
	 * --- in either case, it needs to be repaired.  See
	 * src/bin/pg_alterckey/README for more details.
	 */
	 ...
	 * shared memory.
	 */
	if (!kmgr_verify_cluster_key(cluster_key, keys_wrap, key_lens, KmgrShmem->intlKeys, nkeys))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("supplied cluster key does not match expected cluster key")));

	/* Output DEKs stored in shared memory for testing purposes */
	/* CFE DEBUG */	fputc('\n', stderr);
	for (int id = 0; id < KMGR_MAX_INTERNAL_KEYS; id++)
	{
		char str[MAXPGPATH];
		...
		out_len = pg_hex_encode((char *)(KmgrShmem->intlKeys[id].key),
								 KmgrShmem->intlKeys[id].klen, str, MAXPGPATH - 1);
		str[out_len] = '\0';
		fprintf(stderr,
