...
pg_pwrite_zeros(int fd, size_t size)
{
	 ...
	 *
	 * XXX: Increasing zero buffer size to more than BLCKSZ bytes might improve
	 * write performance on some platforms. However, no such evidence is found
	 * so far. Hence we stick to zero buffer of size BLCKSZ bytes for now.
	 */
