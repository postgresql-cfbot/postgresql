...
trackDroppedObjectsNeeded(void)
{
	...
	return list_length(EventCacheLookup(EVT_SQLDrop)) > 0 ||
		...
		list_length(EventCacheLookup(EVT_TableInitWrite)) > 0;
}
...
publication_deparse_ddl_command_end(PG_FUNCTION_ARGS)
{
	...
	slist_foreach(iter, &(currentEventTriggerState->SQLDropList))
	{
		...
		else if (strncmp(obj->objecttype, "sequence", 5) == 0 ||
				 strncmp(obj->objecttype, "schema", 5) == 0 ||
				 strncmp(obj->objecttype, "index", 5) == 0)
