 ...
 * here are 64 and 128.
 * WIP: The paper uses at most 64 for this node size, and that would
 * simplify using node48 for non-SIMD, if needed.
 */
...
static RT_PTR_ALLOC* RT_NODE_INSERT_INNER(RT_RADIX_TREE *tree, RT_PTR_ALLOC *ref, RT_NODE_PTR node,
...
RT_EXTEND_DOWN(RT_RADIX_TREE *tree, RT_PTR_ALLOC *ref, RT_NODE_PTR node, uint64 key, int shift)
{
	 ...
	 * purpose function.
	 */
	(void) RT_NODE_INSERT_INNER(tree, ref, node,
...
static RT_PTR_ALLOC *
RT_RECURSIVE_SET(RT_RADIX_TREE *tree, RT_PTR_ALLOC *ref, RT_NODE_PTR node, uint64 key, int shift, bool *found)
{
	RT_PTR_ALLOC *slot;
	RT_NODE_PTR child;
	...
	if (shift > 0)
	{
		...
		else
		{
			...
			return RT_RECURSIVE_SET(tree, slot, child, key, shift - RT_NODE_SPAN, found);
		}
	}
	else
	{
		...
		else
		{
			*found = false;
			...
			return RT_NODE_INSERT_INNER(tree, ref, node, chunk, RT_INVALID_PTR_ALLOC);
		}
	}
}
...
RT_SET(RT_RADIX_TREE *tree, uint64 key, RT_VALUE_TYPE *value_p)
{
	...
	slot = RT_RECURSIVE_SET(tree, &tree->ctl->root, rootnode,
								key, tree->ctl->start_shift, &updated);
	...
	else
	{
		...
		if (RT_PTR_ALLOC_IS_VALID(*slot))
		{
			/* have a value already, so update it */
			Assert(updated);
			child.alloc = *slot;
			RT_PTR_SET_LOCAL(tree, &child);
		}
