...
ResetVacStats(Relation rel)
{
	...
	if (IsBootstrapProcessingMode())
		pgcform->relfrozenxid = FirstNormalTransactionId;
	else
		pgcform->relfrozenxid = GetOldestNonRemovableTransactionId(rel);
	Assert(TransactionIdIsNormal(pgcform->relfrozenxid));
