 ...
 * to each List element in turn.  It will be NULL after normal exit from
 * the loop, but an early "break" will leave it pointing at the current
 * List element.
 *
 * Beware that the List reference "l" is evaluated each time through the loop,
 ...
 */
#define for_each_cell(cell, l, initcell)	\
	for ((cell) = (initcell); (cell) != NULL; (cell) = lnext(l, cell))
 ...
 *
 * Note: for three or more lists, it's likely better to use a single integer
 * loop index and fetch values from the lists with list_nth().
 */
...
#define forfour(cell1, list1, cell2, list2, cell3, list3, cell4, list4) \
	for ((cell1) = list_head(list1), (cell2) = list_head(list2), \
		 ...
		 (cell1) = lnext(list1, cell1), (cell2) = lnext(list2, cell2), \
		 (cell3) = lnext(list3, cell3), (cell4) = lnext(list4, cell4))
