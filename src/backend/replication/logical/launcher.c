		...
		nparallelapplyworkers >= max_parallel_apply_workers_per_subscription)
	{
		...
		ereport(LOG,
				(errcode(ERRCODE_CONFIGURATION_LIMIT_EXCEEDED),
				 ...
				 errhint("You might need to increase max_parallel_apply_workers_per_subscription.")));
...
logicalrep_worker_detach(void)
{
	...
	if (am_leader_apply_worker())
	{
		 ...
		 * receiving the worker termination message and sending it to logs when
		 * the same is already done by the parallel worker.
		 */
...
logicalrep_worker_onexit(int code, Datum arg)
{
	 ...
	 * manually release all locks before the worker exits.
	 */
