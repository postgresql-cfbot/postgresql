						 ...
						 bool xact_boundary)
{
	...
	instr_time	total_delta;
	instr_time	self_delta;
	...
	self_delta = localent->counts.self_time;
	INSTR_TIME_SUBTRACT(self_delta, localent->flushed.self_time);
	...
	shfuncent->stats.self_time += INSTR_TIME_GET_MICROSEC(self_delta);

	pgstat_unlock_entry(entry_ref);

	/* Record what has been flushed; counts stays cumulative. */
