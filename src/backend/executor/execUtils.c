...
ExecGetRangeTableRelation(EState *estate, Index rti)
{
	...
	if (rel == NULL)
	{
		...
		{
			/*
			 * In a normal query, unless the planner set the delaylock flag,
			 ...
			 * sufficient to check this only when rellockmode is higher than
			 * the minimum.
			 */
		...
		{
			/*
			 * If we are a parallel worker or delaylock is set, we need to
			 * obtain a lock on the relation.  For parallel workers, this
			 * ensures sane behavior in case the parent process exits before
			 * we do.
			 */
