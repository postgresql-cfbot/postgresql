...
# XXXXXXXX-XXXXXXXX.dd.dd.dd.dd_fork with the components being:
# - WAL LSN in hex format,
# - Decimal database OID.
# - Decimal tablespace OID or 0 for global.
# - Decimal relfilenode.
...
# Verify filename formats matches --save-fullpage.
for my $fullpath (glob "$tmp_folder/raw/*")
{
	...
	my ($hi_lsn_bk, $lo_lsn_bk) = get_block_info($fullpath, $blocksize);
