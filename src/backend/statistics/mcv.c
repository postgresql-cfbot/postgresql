				  ...
				  int *numrows_filtered)
{
	 ...
	 *
	 * Similarly to how we build MCV for single-column case (about half-way
	 * through compute_scalar_stats, we do set the threshold to be 25% more
	 * than average, with at least 2 instances in the sample.
	 *
	 * We however do not enforce the threshold to be at least 1/K (where K is
	 * number of histogram bins), because at this point we don't know if there
	 * will be a histogram at all, or how many bins will it have.
	 *
	 * We're also explicitly decreasing the threshold to 2 when all the
	 * distinct values fit into STATS_MCVLIST_MAX_ITEMS.
	 *
	 * XXX Compute_scalar_stats uses nonnull_cnt/ndistinct, but that is not
	 * quite possible here - we don't have multi-column null_frac and we do
	 * store (combinations of) NULL values in the MCV list. So we use the raw
	 * numrows instead.
	 */
	...
	/*
	 * Walk through the groups and stop once we get below the threshold. The
	 * groups are sorted by size in descending order (thanks to
...
mcv_clauselist_selectivity(PlannerInfo *root, StatisticExtInfo *stat,
						   ...
						   RelOptInfo *rel,
						   Selectivity *lowsel, Selectivity *totalsel,
						   int *mcv_count)
{
	int			i;
	MCVList    *mcv;
