 ...
 *
 * nbtree interface to table_compute_delete_for_tuples().  Deletes a subset of
 * index tuples from caller's deltids array: those that are found dead-to-all
 * in the table.  Only this subset of TIDs is safe to delete from leaf page.
 *
 * Bottom-up index deletion caller provides all the TIDs from the leaf page,
 * without expecting that tableam will check most of them.  This makes sense
 * because the tableam has considerable discretion about which table blocks it
 * checks in bottom-up deletion case.
 *
 * LP_DEAD tuple deletion caller goes through here too, while asking for
 * !bottomup processing from tableam.  It only includes TIDs that point to
 * blocks from index tuples actually marked LP_DEAD (it includes only these
 * tids in delstate.deltids).  This ensures that tableam will visit all table
 * blocks pointed to by LP_DEAD index tuples, without visiting extra table
 * blocks.  This approach allows us to delete some extra index tuples that
 * happen to be dead-to-all (and happen to not have already had their LP_DEAD
 * bit set in passing).  The extra TID checks for LP_DEAD caller are cheap
 * because tableam needs to generate a latestRemovedXid value anyway, which
 * necessitates visiting all table blocks.  (Actually, it's only strictly
 * necessary to get a latestRemovedXid with logged indexes.  LP_DEAD caller
 * nevertheless gets us to try to delete extra TIDs in all cases, to be
 * consistent.)
 *
						  ...
						  TM_IndexDeleteOp *delstate)
{
	...
	latestRemovedXid = table_compute_delete_for_tuples(heapRel, delstate);

	if (delstate->ndeltids == 0)
	{
		/* The tableam has nothing (must be a bottom-up caller) */
	 ...
	 *
	 * Must sort deltids array first.  It must match the order expected by
	 * loop: leaf-page-wise order.  (Note that the array is likely to be much
	 * smaller now, at least in the bottom-up deletion case.)
	 */
