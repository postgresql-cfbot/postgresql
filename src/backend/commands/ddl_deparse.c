				   ...
				   bool do_qual, bool do_with_check)
{
	...
	if (do_qual)
	{
		...
		append_string_object(policyStmt, "USING (%{expression}s)", "expression",
...
new_objtree_for_role_id(Oid roleoid)
{
	...
	if (roleoid != ACL_ID_PUBLIC)
	{
		...
		append_string_object(role, "%{rolename}I", "rolename",
							 pstrdup(rolename));

		ReleaseSysCache(roltup);
	}
	else
		append_string_object(role, "%{rolename}I", "rolename", "public");
...
deparse_AlterFunction(Oid objectId, Node *parsetree)
{
	...
	for (i = 0; i < procForm->pronargs; i++)
	{
		...
		tmpobj = new_objtree_VA("%{type}T", 1,
								"type", ObjTypeObject,
	...
	foreach(cell, node->actions)
	{
		...
		else if (strcmp(defel->defname, "support") == 0)
		{
			...
			append_string_object(tmpobj, "%{name}s", "name",
							  ...
							  ObjectAddress oldschema)
{
	...
	return new_objtree_VA("ALTER %{objtype}s %{identity}s SET SCHEMA %{newschema}I", 3,
						  ...
						  "newschema", ObjTypeString, newschema);
}
...
deparse_GrantStmt(CollectedCommand *cmd)
{
	...
	grantStmt = new_objtree(istmt->is_grant ? "GRANT" : "REVOKE");
	...
	else
	{
		...
		if (istmt->privileges & ACL_VACUUM)
		{
			priv = (char *)privilege_to_string(ACL_VACUUM);
			list = lappend(list, new_string_object(priv));
		}
		if (istmt->privileges & ACL_ANALYZE)
		{
			priv = (char *)privilege_to_string(ACL_ANALYZE);
			list = lappend(list, new_string_object(priv));
		}

		if (!istmt->is_grant && istmt->grant_option)
			append_string_object(grantStmt, "%{grant_option}s", "grant_option",
		...
		if (istmt->col_privs != NIL)
		{
			...
			foreach(ocell, istmt->col_privs)
			{
				...
				append_string_object(grantStmt, "%{priv}s", "priv",
	...
	append_format_string(grantStmt, istmt->is_grant ? "TO" : "FROM");
	...
	if (istmt->grantor_uid)
	{
		...
		rolename = NameStr(((Form_pg_authid) GETSTRUCT(roltup))->rolname);
		append_string_object(grantStmt, "GRANTED BY %{rolename}s",
							 "rolename", rolename);
		ReleaseSysCache(roltup);
	}
	...
	foreach(cell, node->options)
	{
		...
		if (strcmp(elem->defname, "restrict") == 0)
		{
			...
			if (OidIsValid(oprForm->oprrest))
				...
				append_string_object(tmpobj, "%{procedure}s", "procedure",
									 "NONE");
		}
	...
	foreach(cell, cmd->d.opfam.operators)
	{
		...
		if (!stmt->isDrop)
		{
			if (oper->sortfamily == InvalidOid)
				append_string_object(tmpobj, "%{purpose}s", "purpose",
									 "FOR SEARCH");
			else
			{
				ObjTree    *orderbyobj;

				orderbyobj = new_objtree_VA("FOR ORDER BY %{opfamily}D", 1,
										 "opfamily", ObjTypeObject,
										 new_objtree_for_qualname_id(OperatorFamilyRelationId,
																	 oper->sortfamily));
				append_object_object(tmpobj, "%{purpose}s", orderbyobj);
			}
		}
...
deparse_AlterTypeSetStmt(Oid objectId, Node *cmd)
{
	...
	alterType = new_objtree_VA("ALTER TYPE %{identity}D SET", 1,
							   "identity", ObjTypeObject,
...
deparse_CompositeTypeStmt(Oid objectId, Node *parsetree)
{
	...
	composite = new_objtree_VA("CREATE TYPE %{identity}D AS (%{columns:, }s)", 2,
							   "identity", ObjTypeObject,
							   new_objtree_for_qualname_id(TypeRelationId,
														   objectId),
...
deparse_CreateConversion(Oid objectId, Node *parsetree)
{
	...
	ccStmt = new_objtree_VA("CREATE %{default}s CONVERSION %{identity}D FOR %{source}L TO %{dest}L FROM %{function}D", 5,
...
deparse_CreateExtensionStmt(Oid objectId, Node *parsetree)
{
	...
	extStmt = new_objtree_VA("CREATE EXTENSION %{if_not_exists}s %{name}I", 2,
...
deparse_FdwOptions(List *options, char *colname)
{
	...
	if (colname)
		tmp = new_objtree_VA("ALTER COLUMN %{column}I", 1,
...
deparse_CreateRangeStmt(Oid objectId, Node *parsetree)
{
	...
	range = new_objtree_VA("CREATE TYPE %{identity}D AS RANGE", 1,
						   "identity", ObjTypeObject,
						   new_objtree_for_qualname_id(TypeRelationId, objectId));
...
deparse_AlterEnumStmt(Oid objectId, Node *parsetree)
{
	...
	alterEnum = new_objtree_VA("ALTER TYPE %{identity}D", 1,
							   "identity", ObjTypeObject,
							   new_objtree_for_qualname_id(TypeRelationId,
														   objectId));
	...
	if (!node->oldVal)
	{
		...
		append_string_object(alterEnum, "%{if_not_exists}s", "if_not_exists",
							 node->skipIfNewValExists ? "IF NOT EXISTS" : "");

		append_string_object(alterEnum, "%{value}L", "value", node->newVal);
		...
		if (node->newValNeighbor)
		{
			...
			append_string_object(alterEnum, "%{neighbor}L", "neighbor",
								 node->newValNeighbor);
		}
	}
	else
	{
		append_string_object(alterEnum, "RENAME VALUE %{value}L TO", "value",
							 node->oldVal);
		append_string_object(alterEnum, "%{newvalue}L", "newvalue",
							 node->newVal);
	}
...
deparse_CreateCastStmt(Oid objectId, Node *parsetree)
{
	...
	if (node->inout)
		append_string_object(createCast, "%{mechanism}s", "mechanism",
							 "WITH INOUT");
	...
	append_string_object(createCast, "%{context}s", "context", context);
		...
		switch (subcmd->subtype)
		{
			...
			case AT_ColumnDefault:
				...
				else
				{
					...
					append_string_object(tmpobj, "%{definition}s", "definition",
			...
			case AT_DropExpression:
				tmpobj = new_objtree_VA("ALTER COLUMN %{column}I DROP EXPRESSION %{if_exists}s", 3,
			...
			case AT_AlterColumnType:
				{
					...
					if (!istype)
					{
						...
						if (def->raw_default)
						{
							...
							append_string_object(tmpobj2, "%{expression}s", "expression", sub->usingexpr);
						}
						else
							append_bool_object(tmpobj2, "present", false);
			...
			case AT_ReplicaIdentity:
				...
				switch (((ReplicaIdentityStmt *) subcmd->def)->identity_type)
				{
					case REPLICA_IDENTITY_DEFAULT:
						append_string_object(tmpobj, "%{ident}s", "ident",
											 "DEFAULT");
						break;
			...
			case AT_DropIdentity:
				...
				append_string_object(tmpobj, "%{if_exists}s",
									 "if_exists",
									 subcmd->missing_ok ? "IF EXISTS" : "");
				  ...
				  ColumnDef *coldef, bool is_alter, List **exprs)
{
	...
	if (!composite)
		append_string_object(column, "STORAGE %{colstorage}s", "colstorage",
							 get_type_storage(attrForm->attstorage));

	/* USING clause */
	tmpobj = new_objtree("COMPRESSION");
	if (coldef->compression)
		append_string_object(tmpobj, "%{compression_method}I",
	...
	if (!composite)
	{
		...
		append_string_object(column, "%{not_null}s", "not_null",
			...
			coldef->generated != ATTRIBUTE_GENERATED_STORED)
		{
			...
			append_string_object(tmpobj, "%{default}s", "default", defstr);
		}
		else
			append_bool_object(tmpobj, "present", false);
		...
		if (coldef->generated == ATTRIBUTE_GENERATED_STORED)
		{
			...
			defstr = RelationGetColumnDefault(relation, attrForm->attnum,
											  dpcontext, exprs);
			append_string_object(tmpobj, "%{generation_expr}s STORED",
...
deparse_ColumnDef_typed(Relation relation, List *dpcontext, ColumnDef *coldef)
{
	...
	column = new_objtree_VA("%{name}I WITH OPTIONS %{not_null}s", 3,
							"type", ObjTypeString, "column",
							"name", ObjTypeString, coldef->colname,
...
deparse_ColumnIdentity(Oid seqrelid, char identity, bool alter_table)
{
	...
	if (identity == ATTRIBUTE_IDENTITY_ALWAYS)
		append_string_object(identobj, objfmt, "option", "ALWAYS");
	else if (identity == ATTRIBUTE_IDENTITY_BY_DEFAULT)
		append_string_object(identobj, objfmt, "option", "BY DEFAULT");
	else
...
deparse_CreateDomain(Oid objectId, Node *parsetree)
{
	...
	if (defval)
		append_string_object(createDomain, "DEFAULT %{default}s", "default",
							 defval);
...
deparse_CreateFunction(Oid objectId, Node *parsetree)
{
	...
	foreach(cell, node->parameters)
	{
		...
		append_string_object(paramobj, "%{mode}s", "mode",
	...
	if (table_params == NULL)
	{
		...
		append_string_object(tmpobj, "%{setof}s", "setof",
	...
	else
	{
		...
		for (; table_params != NULL; table_params = lnext(node->parameters, table_params))
		{
			...
			paramobj = new_objtree("");
			append_string_object(paramobj, "%{name}I", "name", param->name);
			append_object_object(paramobj, "%{type}T",
								 new_objtree_for_type(typarray[typnum++], -1));
	...
	append_string_object(createFunc, "LANGUAGE %{language}I", "language",
						 NameStr(langForm->lanname));

	if (isfunction)
	{
		append_string_object(createFunc, "%{window}s", "window",
							 procForm->prokind == PROKIND_WINDOW ? "WINDOW" : "");
		append_string_object(createFunc, "%{volatility}s", "volatility",
		...
		append_string_object(createFunc, "%{leakproof}s", "leakproof",
							 procForm->proleakproof ? "LEAKPROOF" : "");
		append_string_object(createFunc, "%{strict}s", "strict",
	...
	if (procForm->prolang == SQLlanguageId && !isnull)
	{
		...
		append_string_object(createFunc, "%{definition}s", "definition",
							 buf.data);
	}
	else if (probin == NULL)
	{
		append_string_object(createFunc, "AS %{definition}L", "definition",
							 source);
	}
	else
	{
		append_string_object(createFunc, "AS %{objfile}L", "objfile", probin);
		append_string_object(createFunc, ", %{symbol}L", "symbol", source);
	}
...
deparse_CreateStatisticsStmt(Oid objectId, Node *parsetree)
{
	...
	if (!isnull)
	{
		...
		if (indoption->dim1)
		{
			...
			append_string_object(createStat, "%{comma}s", "comma",
								 statexprs ? "," : "ON");
...
deparse_CreateForeignServerStmt(Oid objectId, Node *parsetree)
{
	...
	append_string_object(createServer, "FOREIGN DATA WRAPPER %{fdw}I", "fdw",
						 node->fdwname);
...
deparse_CreateLangStmt(Oid objectId, Node *parsetree)
{
	...
	createLang = new_objtree_VA("CREATE %{or_replace}s %{trusted}s LANGUAGE %{identity}s", 3,
								"or_replace", ObjTypeString,
								node->replace ? "OR REPLACE" : "",
...
deparse_CreateSeqStmt(Oid objectId, Node *parsetree)
{
	...
	createSeq = new_objtree_VA("CREATE %{persistence}s SEQUENCE %{identity}D", 2,
...
deparse_CreateStmt(Oid objectId, Node *parsetree)
{
	...
	if (node->tablespacename)
		append_string_object(tmpobj, "%{tablespace}I", "tablespace",
							 node->tablespacename);
	else
	{
	...
	if (node->accessMethod)
		append_string_object(tmpobj, "%{access_method}I", "access_method",
							 node->accessMethod);
	else
	{
...
deparse_CreateTableAsStmt_vanilla(Oid objectId, Node *parsetree)
{
	...
	append_string_object(createStmt, "AS %{query}s", "query",
...
deparse_CreateTrigStmt(Oid objectId, Node *parsetree)
{
	...
	append_string_object(trigger, "%{time}s", "time", trigtiming);
	...
	append_string_object(trigger, "FOR EACH %{for_each}s", "for_each",
	...
	if (node->whenClause)
	{
		...
		append_string_object(tmpobj, "(%{clause}s)", "clause",
...
deparse_DefElem(DefElem *elem, bool is_reset)
{
	...
	if (!is_reset)
		append_string_object(set, "= %{value}L", "value",
							 elem->arg ? defGetString(elem) :
							 defGetBoolean(elem) ? "TRUE" : "FALSE");
...
deparse_FunctionSet(VariableSetKind kind, char *name, char *value)
{
	...
	else if (kind == VAR_SET_VALUE)
	{
		...
		if (GetConfigOptionFlags(name, true) & GUC_LIST_QUOTE)
			append_string_object(obj, "TO %{set_value}s", "set_value", value);
		else
			append_string_object(obj, "TO %{set_value}L", "set_value", value);
	}
	else
...
deparse_IndexStmt(Oid objectId, Node *parsetree)
{
	...
	tmpobj = new_objtree("WITH");
	if (reloptions)
		append_string_object(tmpobj, "(%{opts}s)", "opts", reloptions);
	else
		append_bool_object(tmpobj, "present", false);
	...
	tmpobj = new_objtree("TABLESPACE");
	if (tablespace)
		append_string_object(tmpobj, "%{tablespace}s", "tablespace", tablespace);
	else
		append_bool_object(tmpobj, "present", false);
	...
	if (whereClause)
		append_string_object(tmpobj, "%{where}s", "where", whereClause);
	else
		append_bool_object(tmpobj, "present", false);
...
deparse_OnCommitClause(OnCommitAction option)
{
	...
	{
		case ONCOMMIT_DROP:
			append_string_object(oncommit, "%{on_commit_value}s",
		...
		case OBJECT_COLUMN:
			...
			if (node->renameType == OBJECT_ATTRIBUTE)
			{
				...
				append_object_object(renameStmt, "%{identity}D",
									 new_objtree_for_qualname(schemaId,
															  node->relation->relname));
				append_string_object(renameStmt,
									 "RENAME ATTRIBUTE %{colname}I", "colname",
									 node->subname);
			}
			else
			{
				fmtstr = psprintf("ALTER %s", stringify_objtype(node->relationType, false));
				...
				append_object_object(renameStmt, "%{identity}D",
									 new_objtree_for_qualname(schemaId,
															  node->relation->relname));
				append_string_object(renameStmt, "RENAME COLUMN %{colname}I",
									 "colname", node->subname);
			}

			append_string_object(renameStmt, "TO %{newname}I", "newname",
								 node->newname);
...
deparse_Seq_OwnedBy(Oid sequenceId, bool alter_table)
{
	...
	while (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		...
		append_string_object(tmpobj, "attrname", "attrname", colname);
...
{
	ObjTree    *storage;
	storage = new_objtree_VA("STORAGE = %{value}s", 2,
							 "clause", ObjTypeString, "storage",
...
deparse_CommentStmt(ObjectAddress address, Node *parsetree)
{
	...
	append_string_object(comment, "%{identity}s", "identity", identity);
