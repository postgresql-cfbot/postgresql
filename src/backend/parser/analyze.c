...
transformLetStmt(ParseState *pstate, LetStmt * stmt)
{
	...
	/*
	 * The LET statements suppports two syntaxes: LET var = expr and
	 * LET var = DEFAULT. In first case, the expression is of SelectStmt
	 * node and it is transformated to query (SELECT) by usual way.
	 * Second syntax should be transformed differently. It is more cleaner
	 * do this transformation here (like special case), because we don't
	 * need to enhance SelectStmt about fields necessary for this
	 * transformation somwehere in SelectStmt transformation. Isn't to
	 * necessary to uglify the SelectStmt transformation. This is special
	 * case of LET statement, not SelectStmt.
	 */
	if (stmt->set_default)
	{
		 ...
		 * The SetToDefault is replaced by defexpr by rewriter in
	...
	/*
	 * Save target session variable id. This value is used
	 * multiple times: by query rewriter (for getting related
	 * defexpr), by planner (for acquiring AccessShareLock on
	 * target variable), and by executor (we need to know
	 * target variable id).
	 */
	 ...
	 * return the query because we don't want to use a utilityStmt wrapper node.
	 */
