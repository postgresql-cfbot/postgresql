...
# Test the vacuum_interrupt_count counter of pg_stat_database.
#
# vacuum_interrupt_count records how many times a vacuum in the database was
# interrupted by an error.  We provoke that by starting a vacuum that sleeps
# at its cost-based delay points and canceling it, with pg_cancel_backend(),
...
is( $node->safe_psql(
		'postgres', qq[
SELECT vacuum_interrupt_count > 0 FROM pg_stat_database WHERE datname = current_database()]),
	't',
	'vacuum_interrupt_count advanced in pg_stat_database');
