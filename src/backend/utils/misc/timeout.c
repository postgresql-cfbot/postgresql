		...
		if (num_active_timeouts > 0)
		{
				   ...
				   now >= active_timeouts[0]->fin_time)
			{
				...
				if (this_timeout->interval_in_ms > 0)
				{
					 ...
					 * late that we missed an entire cycle, or if the system
					 * clock has gone backwards, fall back to scheduling based
					 * on the actual firing time.
					 */
					new_fin_time =
 ...
 */
void
enable_timeout_every(TimeoutId id, int delay_ms)
{
	TimestampTz now;
	TimestampTz fin_time;
