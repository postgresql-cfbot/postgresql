 ...
 * deadlocks that do not occur on the publisher side. This can happen even
 * without parallel mode when there are concurrent operations on the
 * subscriber. In order to detect the deadlocks among leader and parallel apply
 * workers, we need to ensure that we wait using lmgr locks. An alternative
 * approach could be to not allow parallelism when the schema of tables is
 * different between the publisher and subscriber but that would be too
 * restrictive and would require the publisher to send much more information
 * than it is currently sending.
 *
 ...
 * STREAM_PREPARE) and release it immediately.
 *
 * The lock graph for the above example will look as follows:
...
pa_setup_dsm(ParallelApplyWorkerInfo *winfo)
{
	...
	shared->pending_stream_count = 0;
	shared->pa_wait_for_stream = false;
...
pa_launch_parallel_worker(void)
{
	...
	bool		launched;
	ParallelApplyWorkerInfo *winfo;
	List	   *active_workers = list_copy(ParallelApplyWorkerPool);
	ListCell   *lc;
	...
	foreach(lc, active_workers)
	{
		...
		/*
		 * We first try to free the worker to improve our chances of finding
		 * one that is not in use. Normally, we free the worker after ensuring
		 * that the transaction is committed by the parallel worker but for
		 * rollbacks, we don't wait for the transaction to finish so can't
		 * free the worker information immediately.
		 */
...
/*
 * Detach from the error_mq_handle for each parallel apply worker.
 */
void
...
pa_stop_idle_workers(void)
{
	...
	foreach(lc, active_workers)
	{
		...
		/*
		 * We first try to free the worker that may be in use. Normally, we
		 * free the worker after ensuring that the transaction is committed by
		 * the parallel worker but for rollbacks, we don't wait for the
		 * transaction to finish so can't free the worker information
		 * immediately.
		 */
		if (pa_free_worker(winfo, winfo->shared->xid))
			continue;
...
ParallelApplyWorkerMain(Datum main_arg)
{
	 ...
	 * leader, detects a change in subscription information, or when there
...
#define SHM_SEND_TIMEOUT_MS		(10000 - SHM_SEND_RETRY_INTERVAL_MS)
	...
	for (;;)
	{
		bool force_serialize = (stream_serialize_threshold != -1 &&
		...
		else if (stream_serialize_threshold != 0)
...
pa_wait_for_xact_finish(ParallelApplyWorkerInfo *winfo)
{
	 ...
	 * deadlock among leader and parallel apply workers. Refer to the comments
	 * atop this file.
	 */
	pa_lock_transaction(winfo->shared->xid, AccessExclusiveLock);
	pa_unlock_transaction(winfo->shared->xid, AccessExclusiveLock);
...
/*
 * Set an enum indicating the transaction state for the given parallel apply
 * worker.
 */
void
...
/*
 * Get an enum indicating the transaction state for the given parallel apply
 * worker.
 */
...
/*
 * Increment the number of pending streaming blocks.
 */
void
pa_increment_stream_block(ParallelApplyWorkerShared *wshared)
{
	while (1)
	{
		...
		/*
		 * Don't try to increment the count if the parallel apply worker is
		 * taking the stream lock. Otherwise, there would be a race condition
		 * that the parallel apply worker checks there is no pending streaming
		 * block and before it actually starts waiting on a lock, the leader
		 * sends another streaming block and take the stream lock again. In
		 * this case, the parallel apply worker will start waiting for the next
		 * streaming block whereas there is actually a pending streaming block
		 * available.
		 */
 ...
 */
void
pa_decr_and_wait_stream_block(ParallelApplyWorkerShared *wshared)
{
	Assert(am_parallel_apply_worker());

	/*
	 * Set the flag to indicate that we are going to take the stream lock.
	 */
	SpinLockAcquire(&wshared->mutex);
	Assert(!wshared->pa_wait_for_stream);
	wshared->pa_wait_for_stream = true;
	SpinLockRelease(&wshared->mutex);

	if (--wshared->pending_stream_count == 0)
	{
		pa_lock_stream(wshared->xid, AccessShareLock);
		pa_unlock_stream(wshared->xid, AccessShareLock);
	}

	SpinLockAcquire(&wshared->mutex);
	wshared->pa_wait_for_stream = false;
	SpinLockRelease(&wshared->mutex);
}
