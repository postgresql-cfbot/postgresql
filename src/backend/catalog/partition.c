 ...
 *
 * Merge NULL partitions, if any. Find the index of merged partition to which
 * the NULL values would belong in the join result. If one joining relation has
 * a NULL partition but not the other, try matching it with the default
 * partition from the other relation since the default partition may have rows
 * with NULL partition key. We can eliminate a NULL partition when it appears
 * only on the inner side of the join and the outer side doesn't have a default
 * partition.
 *
					  ...
					  int *next_index, int *null_index, int *default_index)
{
	...
	else if (outer_has_null && !inner_has_null)
	{
		...
		*null_index = -1;
		if (inner_has_default)
		{
			...
			/*
			 * If the default partition is on the outer side, the resultant
			 * partition acts as the default partition of the result.
			 */
			if (jointype == JOIN_FULL)
			{
				if (*default_index < 0)
				...
				{
					/*
					 * Ended up with multiple default partitions in join,
					 * can't merge.
					 */
					merged = false;
				}
				...
				*null_index = merged_index;
				 ...
				 jointype == JOIN_ANTI)
		{
			if (outer_mmap[outer_ni] < 0)
			{
				outer_mmap[outer_ni] = *next_index;
				*next_index = *next_index + 1;
			}
	...
	else if (!outer_has_null && inner_has_null)
	{
		...
		*null_index = -1;
		...
		{
			if (inner_mmap[inner_ni] < 0)
			{
				inner_mmap[inner_ni] = *next_index;
				*next_index = *next_index + 1;
			}
