...
transformColumnRef(ParseState *pstate, ColumnRef *cref)
{
	...
	if (OidIsValid(varid))
	{
		...
		if (node != NULL)
		{
			 ...
			 * alias same like variable name. In this case, when we found
			 * column reference, and we found reference to possible composite
			 * variable, but the variable is not composite, then we can ignore
			 * the variable as simply improper, and we use column reference
			 * only.
			 */
			if (attrname)
			{
