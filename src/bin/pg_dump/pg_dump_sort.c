					 ...
					 int nLoop)
{
	...
	if (i >= nLoop)
	{
		pg_log_warning(ngettext("there are circular foreign-key constraints on this table:",
								...
								nLoop));
		for (i = 0; i < nLoop; i++)
			pg_log_warning("  %s", loop[i]->name);
		pg_log_warning("You might not be able to restore the dump without using --disable-triggers or temporarily dropping the constraints.");
		pg_log_warning("Consider using a full dump instead of a --data-only dump to avoid this problem.");
		if (nLoop > 1)
