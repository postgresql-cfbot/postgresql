					  ...
					  JsonPathItem *jsp, bool not)
{
	...
	switch (jsp->type)
	{
		...
		case jpiNotEqual:
			 ...
			 * '!(path != scalar)' is not equivalent to 'path == scalar' in
			 * the general case because of sequence comparison semantics:
			 ...
			 * are not supported by the both jsonb opclasses.  However in
			 * strict mode we could omit 'EMPTY(path)' part if the path can
			 * return exactly one item (it does not contain wildcard accessors
			 * or item methods like .keyvalue() etc.).
			 */
			return NULL;
