...
static HashJoinTuple ExecParallelHashLoadTuple(HashJoinTable hashtable,
						  MinimalTuple tuple,
						  dsa_pointer *shared);
...
static HashMemoryChunk ExecParallelHashPopChunkQueue(HashJoinTable table,
							  dsa_pointer *shared);
static bool ExecParallelHashPreallocate(HashJoinTable hashtable,
							int batchno,
							size_t size);
...
ExecHashTableCreate(HashState *state, List *hashOperators, bool keepNulls)
{
	...
	if (hashtable->parallel_state)
	{
			...
			BarrierArriveAndWait(build_barrier, WAIT_EVENT_HASH_BUILD_ELECTING))
		{
			...
			ExecParallelHashTableAllocate(hashtable, 0);
		}
	...
	while ((chunk = ExecParallelHashPopChunkQueue(hashtable, &chunk_shared)))
	{
		...
		while (idx < chunk->used)
		{
			...
			if (batchno == 0)
			{
				...
				copyTuple =
					ExecParallelHashLoadTuple(hashtable, tuple, &shared);
...
retry:
	...
	if (batchno == 0)
	{
		...
		hashTuple = ExecParallelHashLoadTuple(hashtable, tuple, &shared);
		if (hashTuple == NULL)
			goto retry;
	...
	else
	{
		...
		{
			if (!ExecParallelHashPreallocate(hashtable, batchno, tuple_size))
				goto retry;
		}
 ...
 * to other batches and should only be called with tuples that belong in the
 * current batch.
 */
void
ExecParallelHashTableLoad(HashJoinTable hashtable,
						  TupleTableSlot *slot,
						  uint32 hashvalue)
{
	MinimalTuple tuple = ExecFetchSlotMinimalTuple(slot);
...
/*
 * Load a tuple into shared dense storage.  This is equivalent to dense_alloc
 * + memcpy, but for Parallel Hash.
 *
 ...
 */
void
ExecParallelHashTableAllocate(HashJoinTable hashtable, int batchno)
{
 ...
 * packed into memory chunks differently by ExecParallelHashLoadTuple(), but
 ...
 */
static bool
ExecParallelHashPreallocate(HashJoinTable hashtable, int batchno, size_t size)
{
	ParallelHashJoinState *pstate = hashtable->parallel_state;
