...
pgstat_flush_pending_entries(bool nowait)
{
	 ...
	 * Processing a pending entry may add further pending entries to the end
	 * of the list, or move an already-passed entry there (when a callback
	 * accumulates into a dependent entry that was already flushed in this
	 * pass), so a simple iteration won't do.
	 *
	 * Further complicating matters is that we want to delete the current
	 * entry from the list once it is fully flushed, though mid-transaction a
	 * fully flushed entry is retained and only deleted at a transaction
	 * boundary.
	 *
	 * So we just keep track of the next pointer in each loop iteration.
	 *
	 * NOTE: a callback must not create a cycle by accumulating into a
	 * dependent entry whose own callback accumulates back into it, directly
	 * or indirectly; that would re-queue forever.  CHECK_FOR_INTERRUPTS()
	 * below keeps such a pathological cycle cancellable.
	 */
	if (!dlist_is_empty(&pgStatPending))
