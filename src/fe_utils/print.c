					   ...
					   FILE *fout, bool is_pager)
{
	...
	if (!is_pager)
	{
		...
		/*
		 * Wrapping can add extra output lines that count_table_lines() can
		 * only account for if it has wrap widths.  Vertical output uses the
		 * same data width for every field, so use dwidth for every column.
		 */
