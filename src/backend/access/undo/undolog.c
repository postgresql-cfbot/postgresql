...
static void free_undo_log_slot(UndoLogSlot *log, UndoLogNumber logno);
				...
				UndoRecPtr *prevlog_xact_start)
{
 ...
 retry:
	...
	/* Capture end pointer while we have the lock. */
	end = slot->meta.end;
	...
	/*
	 * We don't need to acquire meta_lock to read meta.insert, because we're
	 * attached.  The end pointer was captured while holding meta_lock, and
	 * extend_undo_log() will check it again (a concurrency UndoLogDiscard()
	 * might recycle a segment and move end).
	 */
	if (unlikely(new_insert > end || switch_requested))
	{
...
UndoLogDiscard(UndoRecPtr discard_point, TransactionId xid)
{
	...
	/* Perform any file system operations required to advance 'begin'. */
	new_offset = old_end;
		 ...
		 old_offset += UndoLogSegmentSize)
	{
		...
		if (recycle > 0)
		{
			...
			{
				int elevel = errno == ENOENT ? LOG : ERROR;
 ...
 */
static void
free_undo_log_slot(UndoLogSlot *slot, UndoLogNumber logno)
{
...
undolog_xlog_discard(XLogReaderState *record)
{
	...
	/*
	 * We're about to discard undologs. In Hot Standby mode, ensure that there
	 * are no queries running which need to get tuple from discarded undo.
	 *
	...
	new_offset = old_end;
	for (old_offset = old_begin;
		 old_offset < xlrec->begin;
		 old_offset += UndoLogSegmentSize)
	{
		char old_path[MAXPGPATH];

		/* Tell the checkpointer that the file is going away. */
		undofile_forget_sync(xlrec->logno, old_offset / UndoLogSegmentSize,
							 slot->meta.tablespace);

		UndoLogSegmentPath(xlrec->logno, old_offset / UndoLogSegmentSize,
						   slot->meta.tablespace, old_path);

		/* Try to recycle a segment if was can, to advance new_offset. */
		if (new_offset < xlrec->end)
		{
			char	new_path[MAXPGPATH];

			UndoLogSegmentPath(xlrec->logno, new_offset / UndoLogSegmentSize,
							   slot->meta.tablespace, new_path);
			if (rename(old_path, new_path) == 0)
			{
				elog(DEBUG1, "recycled undo segment \"%s\" -> \"%s\"",
		...
		else
		{
			if (unlink(old_path) == 0)
				...
				ereport(LOG,
						(errcode_for_file_access(),
						 "could not unlink \"%s\": %m", old_path));
		}
	}

	/* Now create any files that we didn't manage to rename into place. */
	while (new_offset < xlrec->end)
	{
		allocate_empty_undo_segment(xlrec->logno, slot->meta.tablespace, new_offset);
		new_offset += UndoLogSegmentSize;
	}
	 ...
	 * containing directory.
	 */
	if (xlrec->begin != old_begin)
		undofile_request_sync_dir(slot->meta.tablespace);

	/* Discard buffers. */
	discard_undo_buffers(xlrec->logno, old_discard, xlrec->discard,
						 xlrec->entirely_discarded);

	/* Update shmem. */
	LWLockAcquire(&slot->meta_lock, LW_EXCLUSIVE);
	slot->meta.begin = xlrec->begin;
	slot->meta.discard = xlrec->discard;
	slot->meta.end = xlrec->end;
	LWLockRelease(&slot->meta_lock);

	/* If we discarded everything, the slot can be given up. */
	if (xlrec->entirely_discarded && xlrec->begin == xlrec->end)
		free_undo_log_slot(slot, xlrec->logno);
}
