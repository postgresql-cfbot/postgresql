 ...
 * In a parallel-aware hash join, there is also a shared state machine which
			...
			switch (BarrierAttach(batch_barrier))
			{
				case PHJ_BATCH_ELECTING:
					...
					if (BarrierArriveAndWait(batch_barrier,
						...
						ExecParallelHashTableAllocate(hashtable, batchno);
					/* Fall through. */
				...
				case PHJ_BATCH_LOADING:
														   ...
														   &hashvalue)))
					{
						slot = ExecStoreMinimalTuple(tuple,
													 hjstate->hj_HashTupleSlot,
													 false);
						ExecParallelHashTableLoad(hashtable, slot, hashvalue);
					}
					sts_end_parallel_scan(hashtable->batches[batchno].inner_tuples);
					BarrierArriveAndWait(batch_barrier,
