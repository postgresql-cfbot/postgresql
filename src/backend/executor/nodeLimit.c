	...
	switch (node->lstate)
	{
		...
		case LIMIT_INWINDOW:
			if (ScanDirectionIsForward(direction))
			{
				 ...
				 * machine to LIMIT_WINDOWTIES.If not (nothing was specified, or ONLY was)
				 * return NULL without advancing the subplan or the position variable
				 * but change the state machine to record having done so
				 *
		...
		case LIMIT_WINDOWEND_TIES:
			...
			else
			{
				...
				/*
				 * Get previous tuple from subplan; there should be one !
				 * And change state-machine status.
				 */
				slot = ExecProcNode(outerPlan);
				if (TupIsNull(slot))
					elog(ERROR, "LIMIT subplan failed to run backwards");
