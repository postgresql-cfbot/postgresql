 ...
 * insertion does not have to create and zero-fill segments itself.  An
 * explicit request larger than max_wal_size is rejected unless force is true.
 *
...
pg_wal_preallocate(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Reject an explicit request that does not fit within max_wal_size, rather
	 * than silently carrying out something smaller than was asked for.  The
	 * default request is not limited, since min_wal_size is what the server
	 * already tries to keep.
	 */
	if (!PG_ARGISNULL(0) && !force)
	{
