...
alter table notnull_tbl1 add column c serial, add constraint notnull_tbl1_c_not_null not null c not enforced; -- error
alter table notnull_tbl1 add column c serial not null not enforced; -- error
alter table notnull_tbl1 add column c int generated always as identity not null not enforced; -- error
alter table notnull_tbl1 add column c int not null not enforced not null not enforced, alter column c add generated always as identity; -- error
alter table notnull_tbl1 add column c int not null not enforced enforced; -- error
alter table notnull_tbl1 add column c int not null not enforced null enforced; -- error
alter table notnull_tbl1 add column c int not null not enforced null; -- error

drop table notnull_tbl1;

-- NOT NULL NOT ENFORCED
CREATE TABLE ne_nn_tbl (x int, CONSTRAINT nn NOT NULL x NOT ENFORCED, NOT NULL x ENFORCED); -- error
CREATE TABLE ne_nn_tbl (x int, CONSTRAINT nn NOT NULL x NOT ENFORCED, b int, CONSTRAINT nnb NOT NULL b NOT VALID);
SELECT conname, conrelid::regclass, convalidated, conenforced, pg_get_constraintdef(oid)
FROM pg_constraint
WHERE conrelid = 'ne_nn_tbl'::regclass;

INSERT INTO ne_nn_tbl VALUES (NULL,  1); -- ok
...
ALTER TABLE ne_nn_tbl ADD CONSTRAINT nn NOT NULL x NOT VALID; -- error, because NOT VALID alone imply ENFORCED
