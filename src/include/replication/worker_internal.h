...
typedef struct ParallelApplyWorkerShared
{
	...
	/*
	 * True if the parallel apply worker is going to take or is taking the
	 * stream lock, false otherwise.
	 */
	bool	pa_wait_for_stream;
...
typedef struct ParallelApplyWorkerInfo
{
	...
	/*
	 * The queue used to transfer error messages from the parallel apply
...
extern int	logicalrep_pa_worker_count(Oid subid);
...
extern bool pa_free_worker(ParallelApplyWorkerInfo *winfo, TransactionId xid);
...
extern void pa_increment_stream_block(ParallelApplyWorkerShared *wshared);
extern void pa_decr_and_wait_stream_block(ParallelApplyWorkerShared *wshared);
