...
static void setNamespaceVisibilityForRTE(List *namespace, RangeTblEntry *rte,
										 bool rel_visible,
										 bool cols_visible);
static List *expandSourceTL(ParseState *pstate, ParseNamespaceItem *nsitem,
						 ...
						 List **mergeSourceTargetList)
{
	 ...
	 * RangeTableEntry.
	 */
	*mergeSourceTargetList = expandSourceTL(pstate, right_nsitem, 0, -1);
...
transformMergeStmt(ParseState *pstate, MergeStmt *stmt)
{
	 ...
	 * because we perform an JOIN between the target and the source relation
	 * to identify the matching and not-matching rows. If we take the usual
	 * path of expanding the target table's inheritance and create one subplan
	 * per partition, then we we won't be able to correctly identify the
	 * matching and not-matching rows since for a given source row, there may
	 * not be a matching row in one partition, but it may exists in some other
	 ...
	 * checking if the OIDs returned by the two lookups is the same. If not,
 ...
 *
 * This function is very similar to expandNSItemAttrs except that we don't mark
 ...
 */
static List *
expandSourceTL(ParseState *pstate, ParseNamespaceItem *nsitem,
			   int sublevels_up, int location)
{
