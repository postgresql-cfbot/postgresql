 ...
 */
static int
brin_bloom_get_ndistinct(BrinDesc *bdesc)
{
	double ndistinct;
	double	maxtuples;
	...
	ndistinct = BrinGetNDistinctPerRange(bdesc->bd_index);
	 ...
	 */
	ndistinct = Max(ndistinct, BRIN_MIN_NDISTINCT_PER_RANGE);
...
static double
brin_bloom_get_fp_rate(BrinDesc *bdesc)
{
	return BrinGetFalsePositiveRate(bdesc->bd_index);
}
...
brin_bloom_add_value(PG_FUNCTION_ARGS)
{
	...
	{
		filter = bloom_init(brin_bloom_get_ndistinct(bdesc),
							brin_bloom_get_fp_rate(bdesc));
