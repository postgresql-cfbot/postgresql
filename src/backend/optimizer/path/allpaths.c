					...
					Index rti, RangeTblEntry *rte)
{
	...
	foreach(l, root->append_rel_list)
	{
		 ...
		 * deemed dummy, it may fall on nullable side in a child-join, which in
		 * turn may participate in a MergeAppend, where we will need the
...
generate_partition_wise_join_paths(PlannerInfo *root, RelOptInfo *rel)
{
	...
	/*
	 * Nothing to do if the relation is not partitioned. We explicitly check
	 * existence of child-joins since a dummy join relation may have rest of
