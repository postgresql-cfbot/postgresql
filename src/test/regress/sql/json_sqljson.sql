...
SELECT JSON_EXISTS(json '1', '$[0]' RETURNING bool);
SELECT JSON_EXISTS(json '1', '$[1]' RETURNING bool);
SELECT JSON_EXISTS(json '1', '$[0]' RETURNING int);
SELECT JSON_EXISTS(json '1', '$[1]' RETURNING int);
SELECT JSON_EXISTS(json '1', '$[0]' RETURNING text);
SELECT JSON_EXISTS(json '1', '$[1]' RETURNING text);
SELECT JSON_EXISTS(json '1', 'strict $[1]' RETURNING text FALSE ON ERROR);
SELECT JSON_EXISTS(json '1', '$[0]' RETURNING jsonb);
SELECT JSON_EXISTS(json '1', '$[0]' RETURNING float4);
...
CREATE TABLE test_json_constraints (
	...
	CONSTRAINT test_json_constraint5
		CHECK (JSON_QUERY(js FORMAT JSON, '$.a' RETURNING char(5) OMIT QUOTES EMPTY ARRAY ON EMPTY) >  'a' COLLATE "C")
);
...
CREATE DOMAIN json_test_domain AS text CHECK (value <> 'foo');
...
DROP VIEW json_table_view;
DROP DOMAIN json_test_domain;
