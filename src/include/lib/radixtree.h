...
/* WIP: consider 5 with subclass node2 */
#define RT_FANOUT_4		4
StaticAssertDecl(RT_FANOUT_4 <= 8 - sizeof(RT_NODE),
	"too much struct padding");
...
RT_CHUNK_CHILDREN_ARRAY_COPY(uint8 *src_chunks, RT_PTR_ALLOC *src_children,
						  ...
						  int insertpos, const int count)
{
	/* first copy all old elements -- with a compile-time constant, it should generate better code */
	memcpy(&dst_chunks[0], &src_chunks[0],
			count * sizeof(src_chunks[0]));
	memcpy(&dst_children[0], &src_children[0],
			count * sizeof(src_children[0]));

	/* then the old elements after the insert position */
				...
				uint8 chunk, RT_PTR_ALLOC child, int insertpos)
{
	...
	if (n16->base.n.fanout < RT_FANOUT_16_HI)
	{
		...
		Assert(n16->base.n.count == RT_FANOUT_16_LO);
				...
				uint8 chunk, RT_PTR_ALLOC child)
{
	...
	if (unlikely(RT_NODE_MUST_GROW(n16)))
		RT_GROW_NODE_16(tree, ref, node, chunk, child, insertpos);
	else
	{
				...
				uint8 chunk, RT_PTR_ALLOC child)
{
	...
	{
		RT_GROW_NODE_4(tree, ref, node, chunk, child, insertpos);
		return;
	}

	/* shift chunks and children

		Unfortunately, gcc has gotten too aggressive in turning simple loops
	...
	*/
	...
	/*
	 * Starting at insertpos, unconditionally move 4 bytes up by 1.
		This may spill into the first child, so we save it in a temporary variable to keep it
	   from being overwritten. We don't restore it right away since it, too, may need to be moved up.
	 */
	Assert(n4->base.n.count <= 4);
	Assert((char *) &n4->base.chunks[insertpos] + 4 < (char *) &n4->children[1]);
	tmp = n4->children[0];
	memmove(&(n4->base.chunks[insertpos + 1]),
			&(n4->base.chunks[insertpos]),
			4);

	if (true) // WIP: if node5 with node2 subclass: (likely(n4->base.n.fanout == RT_FANOUT_4_HI))
	{
		/* going above this would break this code, and there is no
		reason to, especially on platforms with SIMD */
		Assert(RT_FANOUT_4 <= 5);

		/*
		 * unconditionaly shift the last child to the last slot.
		 * Needed for insertpos 3 or less, and harmless for insertpos 4.
		 */
		if (RT_FANOUT_4 == 5)
			n4->children[4] = n4->children[3];

		/*
		 * shift up previous two children -- some architectures can use
		 * a single load/store for this. Works for both insertpos 1 and 2.
		 */
		Assert(RT_FANOUT_4 >= 4);
		if (insertpos < 3)
			memmove(&n4->children[2], &n4->children[1],
					2 * sizeof(n4->children[0]));
	}

	/*
	 * Restore first child to its proper position, either 0 or 1.
	 * The compiler should be able to turn this into branch-free code.
	 * WIP: This is all that is needed for a future node2 size class.
	 */
	n4->children[insertpos == 0] = tmp;
