					   ...
					   List *parent_restrictlist)
{
	...
	for (cnt_parts = 0; cnt_parts < joinrel->nparts; cnt_parts++)
	{
		...
		/*
		 * When there are thousands of partitions involved, this loop can
		 * accumulate significant memory usage from objects that are only
		 * needed within the loop.  Free these local objects eagerly at the
