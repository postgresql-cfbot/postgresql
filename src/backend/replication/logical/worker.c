...
static void store_flush_position(XLogRecPtr remote_lsn, XLogRecPtr local_lsn);
...
apply_handle_stream_prepare(StringInfo s)
{
	...
	switch (apply_action)
	{
		...
		case TRANS_PARALLEL_APPLY:
			...
			pa_unlock_transaction(MyParallelShared->xid, AccessShareLock);
	...
	switch (apply_action)
	{
		...
		case TRANS_PARALLEL_APPLY:
			if (first_segment)
			{
				...
				pa_lock_transaction(MyParallelShared->xid, AccessShareLock);
...
apply_handle_stream_stop(StringInfo s)
{
	...
	switch (apply_action)
	{
		...
		case TRANS_PARALLEL_APPLY:
			 ...
			 * applyparallelworker.c.
			 */
			pa_decr_and_wait_stream_block(MyParallelShared);
...
/*
 * Handle STREAM ABORT message when the transaction was serialized to file.
 */
static void
...
apply_handle_stream_abort(StringInfo s)
{
	...
	switch (apply_action)
	{
		...
		case TRANS_LEADER_SEND_TO_PARALLEL:
			 ...
			 * processing the STREAM_ABORT message if it is not already waiting
