 ...
 * to append elements to the List; they are guaranteed to be visited.
 * Also, the current element of the List can be deleted, if you use
 * foreach_delete_current() to do so.
 */
...
{
	ForEachState r = {lst, initcell ? list_cell_number(lst, initcell) : list_length(lst)};
