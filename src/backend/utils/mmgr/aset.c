 ...
 * itself to store the freelist link.  To make it less likely that any write
 * after pfree() bugs corrupt this, we put the link at the very end of the
 * allocated chunk.
 */
#define GetFreeListLink(chkptr, chksz) \
	(AllocFreelistLink *) ((char *) (chkptr) + ALLOC_CHUNKHDRSZ + \
	(chksz) - sizeof(AllocFreelistLink))
	...
	if ((block = set->blocks) != NULL)
	{
		...
		if (availspace < (chunk_size + ALLOC_CHUNKHDRSZ))
		{
			...
			while (availspace >= ((1 << ALLOC_MINBITS) + ALLOC_CHUNKHDRSZ))
			{
				...
				link = GetFreeListLink(chunk, availchunk);
...
AllocSetFree(void *pointer)
{
	...
	else
	{
		...
		Size		chunk_size = GetChunkSizeFromFreelistIdx(fidx);
		AllocFreelistLink *link = GetFreeListLink(chunk, chunk_size);
		...
		/* push this chunk onto the top of the freelist for this chunk size */
		VALGRIND_MAKE_MEM_DEFINED(link, sizeof(AllocFreelistLink));
		...
		/*
		 * Reset requested_size to InvalidAllocSize in chunks that are on
		 * freelist.
		 */
		 ...
		 prevblock = block, block = block->next)
	{
		...
		while (bpoz < block->freeptr)
		{
			...
			else
			{
				...
				chunkblock = (AllocBlock) MemoryChunkGetBlock(chunk);
			}
