...
load_key(char *name, CFArrayRef *out)
{
	...
	if (!check_ssl_key_file_permissions(name, true))
		return;
...
/*
 * This function should return the expected TLS Finished message information
 * from the client, but there is no API for this in Secure Transport. As a
 * result, channel binding is not supported by Secure Transport.
 */
char *
