...
static LLVMValueRef
create_ExecAggTransRepatriate(LLVMModuleRef mod)
{
   LLVMTypeRef sig;
   LLVMValueRef fn;
   LLVMTypeRef param_types[6];

   const char *nm = "ExecAggTransRepatriate";

   fn = LLVMGetNamedFunction(mod, nm);
   if (fn)
	   return fn;

   param_types[0] = LLVMPointerType(TypeSizeT, 0); /* aggstate */
   param_types[1] = LLVMPointerType(TypeSizeT, 0); /* pertrans */
   param_types[2] = TypeSizeT; /* newValue */
   param_types[3] = LLVMInt8Type(); /* newValueIsNull */
   param_types[4] = TypeSizeT; /* oldValue */
   param_types[5] = LLVMInt8Type(); /* oldValueIsNull */
			...
			case EEOP_AGG_PLAIN_TRANS:
				{
					...
					v_aggstatep = LLVMBuildIntToPtr(
						builder,
						LLVMConstInt(TypeSizeT, (uintptr_t) aggstate, false),
						LLVMPointerType(TypeSizeT, 0),
						"");
...

					{
						...
						inline_mod = llvm_module_for_function("AggCheckCallContext");
						if (inline_mod)
						{
							context->inline_modules =
								list_append_unique_ptr(context->inline_modules,
					 ...
					 * without copying it.
					 */
					if ((ExprEvalOp) op->opcode == EEOP_AGG_PLAIN_TRANS)
					{
						LLVMBasicBlockRef b_call = LLVMInsertBasicBlock(opblocks[i + 1], "transcall");
						LLVMBasicBlockRef b_nocall = LLVMInsertBasicBlock(opblocks[i + 1], "transnocall");

						LLVMValueRef v_transvalue, v_transnull;
						LLVMValueRef v_newval;

						LLVMValueRef params[6];

						v_transvalue = LLVMBuildLoad(
							builder, v_transvaluep, "");
						v_transnull = LLVMBuildLoad(
							builder, v_transnullp, "");

						// DatumGetPointer(newVal) != DatumGetPointer(pergroup->transValue))
						LLVMBuildCondBr(
							builder,
							LLVMBuildICmp(builder, LLVMIntEQ, v_transvalue,
										  v_retval, ""),
							b_nocall, b_call);

						LLVMPositionBuilderAtEnd(builder, b_call);
						...
						v_newval = LLVMBuildCall(
							builder,
							create_ExecAggTransRepatriate(mod),
							params, lengthof(params),
							"");
						...
						LLVMPositionBuilderAtEnd(builder, b_nocall);
