...
select json '{}' @? '$.*';
select json '{"a": 1}' @? '$.*';
select json '{"a": {"b": 1}}' @? 'pg $.**{1}';
select json '{"a": {"b": 1}}' @? 'pg $.**{2}';
select json '{"a": {"b": 1}}' @? 'pg $.**{3}';
select json '[]' @? '$[*]';
select json '[1]' @? '$[*]';
select json '[1]' @? '$[1]';
...
select json_path_query('{"a": {"b": 1}}', 'pg $.**');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{0}');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{0 to last}');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{1}');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{1 to last}');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{2}');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{2 to last}');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{3 to last}');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{last}');
select json_path_query('{"a": {"b": 1}}', 'pg $.**.b ? (@ > 0)');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{0}.b ? (@ > 0)');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{1}.b ? (@ > 0)');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{0 to last}.b ? (@ > 0)');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{1 to last}.b ? (@ > 0)');
select json_path_query('{"a": {"b": 1}}', 'pg $.**{1 to 2}.b ? (@ > 0)');
select json_path_query('{"a": {"c": {"b": 1}}}', 'pg $.**.b ? (@ > 0)');
select json_path_query('{"a": {"c": {"b": 1}}}', 'pg $.**{0}.b ? (@ > 0)');
select json_path_query('{"a": {"c": {"b": 1}}}', 'pg $.**{1}.b ? (@ > 0)');
select json_path_query('{"a": {"c": {"b": 1}}}', 'pg $.**{0 to last}.b ? (@ > 0)');
select json_path_query('{"a": {"c": {"b": 1}}}', 'pg $.**{1 to last}.b ? (@ > 0)');
select json_path_query('{"a": {"c": {"b": 1}}}', 'pg $.**{1 to 2}.b ? (@ > 0)');
select json_path_query('{"a": {"c": {"b": 1}}}', 'pg $.**{2 to 3}.b ? (@ > 0)');

select json '{"a": {"b": 1}}' @? 'pg $.**.b ? ( @ > 0)';
select json '{"a": {"b": 1}}' @? 'pg $.**{0}.b ? ( @ > 0)';
select json '{"a": {"b": 1}}' @? 'pg $.**{1}.b ? ( @ > 0)';
select json '{"a": {"b": 1}}' @? 'pg $.**{0 to last}.b ? ( @ > 0)';
select json '{"a": {"b": 1}}' @? 'pg $.**{1 to last}.b ? ( @ > 0)';
select json '{"a": {"b": 1}}' @? 'pg $.**{1 to 2}.b ? ( @ > 0)';
select json '{"a": {"c": {"b": 1}}}' @? 'pg $.**.b ? ( @ > 0)';
select json '{"a": {"c": {"b": 1}}}' @? 'pg $.**{0}.b ? ( @ > 0)';
select json '{"a": {"c": {"b": 1}}}' @? 'pg $.**{1}.b ? ( @ > 0)';
select json '{"a": {"c": {"b": 1}}}' @? 'pg $.**{0 to last}.b ? ( @ > 0)';
select json '{"a": {"c": {"b": 1}}}' @? 'pg $.**{1 to last}.b ? ( @ > 0)';
select json '{"a": {"c": {"b": 1}}}' @? 'pg $.**{1 to 2}.b ? ( @ > 0)';
select json '{"a": {"c": {"b": 1}}}' @? 'pg $.**{2 to 3}.b ? ( @ > 0)';
...
select json '{"c": {"a": 1, "b":1}}' @? '$.* ? (@.a == @.b)';
select json '{"a": 1, "b":1}' @? 'pg $.** ? (@.a == @.b)';
select json '{"c": {"a": 1, "b":1}}' @? 'pg $.** ? (@.a == @.b)';

select json_path_query('{"c": {"a": 2, "b":1}}', 'pg $.** ? (@.a == 1 + 1)');
select json_path_query('{"c": {"a": 2, "b":1}}', 'pg $.** ? (@.a == (1 + 1))');
select json_path_query('{"c": {"a": 2, "b":1}}', 'pg $.** ? (@.a == @.b + 1)');
select json_path_query('{"c": {"a": 2, "b":1}}', 'pg $.** ? (@.a == (@.b + 1))');
select json '{"c": {"a": -1, "b":1}}' @? 'pg $.** ? (@.a == - 1)';
select json '{"c": {"a": -1, "b":1}}' @? 'pg $.** ? (@.a == -1)';
select json '{"c": {"a": -1, "b":1}}' @? 'pg $.** ? (@.a == -@.b)';
select json '{"c": {"a": -1, "b":1}}' @? 'pg $.** ? (@.a == - @.b)';
select json '{"c": {"a": 0, "b":1}}' @? 'pg $.** ? (@.a == 1 - @.b)';
select json '{"c": {"a": 2, "b":1}}' @? 'pg $.** ? (@.a == 1 - - @.b)';
select json '{"c": {"a": 0, "b":1}}' @? 'pg $.** ? (@.a == 1 - +@.b)';
select json '[1,2,3]' @? '$ ? (+@[*] > +2)';
select json '[1,2,3]' @? '$ ? (+@[*] > +3)';
select json '[1,2,3]' @? '$ ? (-@[*] < -2)';
