...
static PGresult *fetch_stream_result(ForeignScanState *node);
...
postgresReScanForeignScan(ForeignScanState *node)
{
	...
	else if (fsstate->fetch_ct_2 > 1)
	{
		...
		{
			save_to_tuplestore(fsstate->conn_state->active_scan, fsstate->conn);
...
init_scan(ForeignScanState *node)
{
	...
	/*
	 * Unreachable: set_streaming_fetch() rejects streaming_fetch without
	 * this.
	 */
	elog(ERROR, "streaming_fetch is not supported by this build of libpq");
#endif
...
fetch_more_data(ForeignScanState *node)
{
	...
	else if (!fsstate->streaming_fetch)
	{
		...
		save_to_tuplestore(fsstate->conn_state->active_scan, conn);
 ...
 */
static PGresult *
fetch_stream_result(ForeignScanState *node)
{
	PgFdwScanState *fsstate = (PgFdwScanState *) node->fdw_state;
	PGconn	   *conn = fsstate->conn;
	PGresult   *res;
	...
	/*
	 * PGRES_TUPLES_OK is the end-of-stream sentinel in chunked mode. Consume
	 * the trailing protocol NULL now and clear active_scan so a concurrent
	 * init_scan does not call save_to_tuplestore on an already-idle
	 * connection.
	 */
	if (PQresultStatus(res) == PGRES_TUPLES_OK)
	{
...
/*
 * In streaming_fetch mode only, save the tuples to tuplestore.
 */
static void
save_to_tuplestore(PgFdwScanState *active_fsstate, PGconn *conn)
{
	...
	{
		/*
		 * This signifies there is nothing to do, all is fetched. But this
	...
	{
		/* Handle any other unexpected result status. */
		pgfdw_report_error(res, conn, active_fsstate->query);
	}
#endif
					   ...
					   int *numSlots)
{
	 ...
	 */
	save_to_tuplestore(fmstate->conn_state->active_scan, fmstate->conn);
...
execute_dml_stmt(ForeignScanState *node)
{
	 ...
	 * the tuples to tuplestore before proceeding further.
	 */
	save_to_tuplestore(dmstate->conn_state->active_scan, dmstate->conn);
...
set_streaming_fetch(DefElem *def, PgFdwRelationInfo *fpinfo)
{
	...
	if (fpinfo->streaming_fetch)
		ereport(ERROR, errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				errmsg("streaming_fetch is not supported by this build of libpq"));
#endif
}
