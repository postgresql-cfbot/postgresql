				...
				CombineRange *cranges, int ncranges)
{
	...
	for (i = 0; i < (ncranges-1); i++)
	{
		...
		r = FunctionCall4Coll(distanceFn, colloid, a1, a2,
							  cranges[0].minval,			/* global min */
							  cranges[ncranges-1].maxval);	/* global max */
...
brin_minmax_multi_distance_float4(PG_FUNCTION_ARGS)
{
	...
	/*
	 * We know the values are boundaries of two adjacent ranges, and we
	 * know that there is a gap between the them.
	 */
	Assert(a1 < a2);
...
brin_minmax_multi_distance_tid(PG_FUNCTION_ARGS)
{
	...
	PG_RETURN_FLOAT8((double) da2 - (double) da1);
}

/*
 * Comutes distance between two numeric values (plain subtraction).
 *
 * To prevent possible overflows, we compute (a2-a1)/(b2-b1) as numeric,
 * and only then convert it to double precision.
 */
Datum
brin_minmax_multi_distance_numeric(PG_FUNCTION_ARGS)
{
	...
	Datum	b1 = PG_GETARG_DATUM(2);
	Datum	b2 = PG_GETARG_DATUM(3);

	/*
	 * We know the values are boundaries of two adjacent ranges, and we
	 * know that there is a gap between the them.
	 */
	Assert(DatumGetBool(DirectFunctionCall2(numeric_lt, a1, a2)));

	/*
	 * The overall range however must not be collapsed, because that
	 * would mean we have many "partial" ranges that are the same.
	 */
	Assert(DatumGetBool(DirectFunctionCall2(numeric_lt, b1, b2)));

	/* And the partial range must be contained in the overall one. */
	Assert((DatumGetBool(DirectFunctionCall2(numeric_le, b1, a1))) &&
		   (DatumGetBool(DirectFunctionCall2(numeric_le, a2, b2))));

	/* Compute (a2-a1)/(b2-b1) to prevent underflows/overflows. */
	distance = DirectFunctionCall2(numeric_div,
							DirectFunctionCall2(numeric_sub, a2, a1),
							DirectFunctionCall2(numeric_sub, b2, b1));

	PG_RETURN_FLOAT8(DirectFunctionCall1(numeric_float8, distance));
}
...
brin_minmax_multi_distance_uuid(PG_FUNCTION_ARGS)
{
	 ...
	 * know that there is a gap between the them.
	 */
	Assert(DatumGetBool(DirectFunctionCall2(uuid_lt, a1, a2)));

	/* compute approximate delta as a double precision value */
	for (i = UUID_LEN-1; i >= 0; i--)
	{
		/* cast to int first, to allow negative results */
...
{
	TimeADT	ta1 = PG_GETARG_TIMEADT(0);
	TimeADT	ta2 = PG_GETARG_TIMEADT(1);
	...
	PG_RETURN_FLOAT8((double) ta2 - (double) ta1);
}
...
{
	TimeTzADT  *ta1 = PG_GETARG_TIMETZADT_P(0);
	TimeTzADT  *ta2 = PG_GETARG_TIMETZADT_P(1);
	...
	PG_RETURN_FLOAT8((double) ta2->time - (double) ta1->time);
}
...
{
	TimeADT		ia1 = PG_GETARG_TIMEADT(0);
	TimeADT		ia2 = PG_GETARG_TIMEADT(1);
	...
	PG_RETURN_FLOAT8((double) ia2 - (double) ia1);
}
...
{
	XLogRecPtr	lsna1 = PG_GETARG_LSN(0);
	XLogRecPtr	lsna2 = PG_GETARG_LSN(1);
	...
	PG_RETURN_FLOAT8(lsna2 - lsna1);
}

/*
 * Compute distance between two macaddr values.
 *
 * mac addresses are treated as 6 unsigned chars, so do the same thing
 * we already do for UUID values.
 */
Datum
...
{
	AbsoluteTime a1 = PG_GETARG_ABSOLUTETIME(0);
	AbsoluteTime a2 = PG_GETARG_ABSOLUTETIME(1);
...
{
	RelativeTime a1 = PG_GETARG_RELATIVETIME(0);
	RelativeTime a2 = PG_GETARG_RELATIVETIME(1);
